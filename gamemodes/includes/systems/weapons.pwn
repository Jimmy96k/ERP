...
// SA-MP 0.3.7 has no RemovePlayerWeapon native (that's open.mp only), so to
// take a single weapon off a player we snapshot everything, reset, and
// re-give every slot except the one being removed.
stock ER_RemovePlayerWeaponSlot(playerid, slot)
{
    new weapons[MAX_WEAPON_SLOTS], ammos[MAX_WEAPON_SLOTS];
    for(new i; i < MAX_WEAPON_SLOTS; i++) GetPlayerWeaponData(playerid, i, weapons[i], ammos[i]);
    ResetPlayerWeapons(playerid);
    for(new i; i < MAX_WEAPON_SLOTS; i++)
    {
        if(i == slot) continue;
        if(weapons[i] > 0) GivePlayerWeapon(playerid, weapons[i], ammos[i]);
    }
    return 1;
}

// Finds a weapon slot the player is currently carrying, matching either a
// numeric GTA weapon ID or a partial weapon name (e.g. "deagle", "9" ).
// Skips empty slots and the fist slot (weapon 0). Returns the slot index or -1.
stock ER_FindPlayerWeaponSlot(playerid, const input[])
{
    if(!input[0]) return -1;
    ...
    for(new slot; slot < MAX_WEAPON_SLOTS; slot++)
    {
        ...
        if(wid > 0 && weapon == wid) return slot;
        GetWeaponName(weapon, wname, sizeof(wname));
        if(strfind(wname, input, true) != -1) return slot;
    }
    return -1;
}
