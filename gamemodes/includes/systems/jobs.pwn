...
// Matrun local limits/dialogs (kept short to avoid Pawn 31-char truncation).
#if !defined MAX_MATRUNS
    #define MAX_MATRUNS 128
#endif
#if !defined DIALOG_MRUN_LIST
    #define DIALOG_MRUN_LIST   8920
#endif
#if !defined DIALOG_MRUN_EDIT
    #define DIALOG_MRUN_EDIT   8921
#endif
#if !defined DIALOG_MRUN_INPUT
    #define DIALOG_MRUN_INPUT  8922
#endif
#if !defined DIALOG_MRUN_DELETE
...
// -----------------------------------------------------------------------------
// Shared Arms Dealer / Craftsman material runs
// Created jobs are still job join points; matruns are separate economy routes.
// -----------------------------------------------------------------------------
enum E_MATRUN_INFO
{
    mrSQLID,
    ...
    mrPickupPointID,
    mrDropoffPointID,
    mrDropoffPointCutPercent,
    mrPickupIcon,
    mrDropoffIcon,
    mrEnabled,
    mrPickupID,
    mrDropoffPickupID,
    Text3D:mrPickupLabelID,
    Text3D:mrDropoffLabelID
};
new Matruns[MAX_MATRUNS][E_MATRUN_INFO];
new MatrunCount;

stock ER_DestroyMatrunWorld(idx)
{
    if(idx < 0 || idx >= MAX_MATRUNS) return 0;
    if(Matruns[idx][mrPickupID]) DestroyDynamicPickup(Matruns[idx][mrPickupID]);
    if(Matruns[idx][mrDropoffPickupID]) DestroyDynamicPickup(Matruns[idx][mrDropoffPickupID]);
    if(Matruns[idx][mrPickupLabelID]) DestroyDynamic3DTextLabel(Matruns[idx][mrPickupLabelID]);
    if(Matruns[idx][mrDropoffLabelID]) DestroyDynamic3DTextLabel(Matruns[idx][mrDropoffLabelID]);
    Matruns[idx][mrPickupID] = 0;
    Matruns[idx][mrDropoffPickupID] = 0;
    Matruns[idx][mrPickupLabelID] = Text3D:0;
    Matruns[idx][mrDropoffLabelID] = Text3D:0;
    return 1;
}

stock ER_ClearMatruns()
{
    for(new i; i < MatrunCount; i++) ER_DestroyMatrunWorld(i);
    MatrunCount = 0;
    return 1;
}

stock ER_CreateMatrunWorld(idx)
{
    new label[256], pointStatus[96];

    ER_FormatPointStatusForMatrun(Matruns[idx][mrPickupPointID], pointStatus, sizeof(pointStatus));
    format(label, sizeof(label), "{FFFF00}%s\n{FFFFFF}%s\nPackages: %d | Price: %s\nUse /getmats", Matruns[idx][mrPickupName], pointStatus, Matruns[idx][mrPackageAmount], ER_FormatMoney(Matruns[idx][mrPackageCost]));
    Matruns[idx][mrPickupID] = CreateDynamicPickup(Matruns[idx][mrPickupIcon], 23, Matruns[idx][mrPickupX], Matruns[idx][mrPickupY], Matruns[idx][mrPickupZ], Matruns[idx][mrPickupVW], Matruns[idx][mrPickupInt]);
    Matruns[idx][mrPickupLabelID] = CreateDynamic3DTextLabel(label, COLOR_YELLOW, Matruns[idx][mrPickupX], Matruns[idx][mrPickupY], Matruns[idx][mrPickupZ] + 0.35, 15.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 1, Matruns[idx][mrPickupVW], Matruns[idx][mrPickupInt]);

    ER_FormatPointStatusForMatrun(Matruns[idx][mrDropoffPointID], pointStatus, sizeof(pointStatus));
    format(label, sizeof(label), "{FFFF00}%s\n{FFFFFF}%s\nReward: %d materials", Matruns[idx][mrDropoffName], pointStatus, Matruns[idx][mrMaterialReward]);
    Matruns[idx][mrDropoffPickupID] = CreateDynamicPickup(Matruns[idx][mrDropoffIcon], 23, Matruns[idx][mrDropoffX], Matruns[idx][mrDropoffY], Matruns[idx][mrDropoffZ], Matruns[idx][mrDropoffVW], Matruns[idx][mrDropoffInt]);
    Matruns[idx][mrDropoffLabelID] = CreateDynamic3DTextLabel(label, COLOR_YELLOW, Matruns[idx][mrDropoffX], Matruns[idx][mrDropoffY], Matruns[idx][mrDropoffZ] + 0.35, 15.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 1, Matruns[idx][mrDropoffVW], Matruns[idx][mrDropoffInt]);
    return 1;
}

stock ER_RefreshMatrunWorld(idx)
{
    if(idx < 0 || idx >= MatrunCount) return 0;
    ER_DestroyMatrunWorld(idx);
    if(Matruns[idx][mrEnabled]) ER_CreateMatrunWorld(idx);
    return 1;
}

stock ER_RefreshMatrunsLinkedToPoint(pointid)
{
    if(pointid <= 0) return 0;
    for(new i; i < MatrunCount; i++)
    {
        if(Matruns[i][mrSQLID] <= 0) continue;
        if(Matruns[i][mrPickupPointID] == pointid || Matruns[i][mrDropoffPointID] == pointid)
        {
            ER_RefreshMatrunWorld(i);
        }
    }
    return 1;
}

stock ER_LoadMatrunRow(row, idx)
{
    cache_get_value_name_int(row, "id", Matruns[idx][mrSQLID]);
    cache_get_value_name(row, "name", Matruns[idx][mrName], 64);
    cache_get_value_name(row, "pickup_name", Matruns[idx][mrPickupName], 64);
    cache_get_value_name(row, "dropoff_name", Matruns[idx][mrDropoffName], 64);
    if(!Matruns[idx][mrPickupName][0]) format(Matruns[idx][mrPickupName], 64, "%s Pickup", Matruns[idx][mrName]);
    if(!Matruns[idx][mrDropoffName][0]) format(Matruns[idx][mrDropoffName], 64, "%s Dropoff", Matruns[idx][mrName]);
    cache_get_value_name_float(row, "pickup_x", Matruns[idx][mrPickupX]);
    cache_get_value_name_float(row, "pickup_y", Matruns[idx][mrPickupY]);
    cache_get_value_name_float(row, "pickup_z", Matruns[idx][mrPickupZ]);
    cache_get_value_name_int(row, "pickup_int", Matruns[idx][mrPickupInt]);
    cache_get_value_name_int(row, "pickup_vw", Matruns[idx][mrPickupVW]);
    cache_get_value_name_float(row, "dropoff_x", Matruns[idx][mrDropoffX]);
    cache_get_value_name_float(row, "dropoff_y", Matruns[idx][mrDropoffY]);
    cache_get_value_name_float(row, "dropoff_z", Matruns[idx][mrDropoffZ]);
    cache_get_value_name_int(row, "dropoff_int", Matruns[idx][mrDropoffInt]);
    cache_get_value_name_int(row, "dropoff_vw", Matruns[idx][mrDropoffVW]);
    cache_get_value_name_int(row, "package_amount", Matruns[idx][mrPackageAmount]);
    cache_get_value_name_int(row, "package_cost", Matruns[idx][mrPackageCost]);
    cache_get_value_name_int(row, "material_reward", Matruns[idx][mrMaterialReward]);
    cache_get_value_name_int(row, "pickup_point_id", Matruns[idx][mrPickupPointID]);
    cache_get_value_name_int(row, "dropoff_point_id", Matruns[idx][mrDropoffPointID]);
    cache_get_value_name_int(row, "dropoff_point_cut_percent", Matruns[idx][mrDropoffPointCutPercent]);
    cache_get_value_name_int(row, "pickup_icon", Matruns[idx][mrPickupIcon]);
    cache_get_value_name_int(row, "dropoff_icon", Matruns[idx][mrDropoffIcon]);
    cache_get_value_name_int(row, "enabled", Matruns[idx][mrEnabled]);
    if(Matruns[idx][mrPackageAmount] <= 0) Matruns[idx][mrPackageAmount] = 10;
    if(Matruns[idx][mrPackageCost] < 0) Matruns[idx][mrPackageCost] = 75;
    if(Matruns[idx][mrMaterialReward] <= 0) Matruns[idx][mrMaterialReward] = 250;
    if(Matruns[idx][mrPickupIcon] <= 0) Matruns[idx][mrPickupIcon] = 1271;
    if(Matruns[idx][mrDropoffIcon] <= 0) Matruns[idx][mrDropoffIcon] = 1239;
    if(Matruns[idx][mrDropoffPointCutPercent] < 0) Matruns[idx][mrDropoffPointCutPercent] = 0;
    if(Matruns[idx][mrDropoffPointCutPercent] > 100) Matruns[idx][mrDropoffPointCutPercent] = 100;
    return 1;
}

stock ER_LoadMatruns()
{
    ER_ClearMatruns();
    mysql_tquery(MainPipeline, "SELECT * FROM `job_matruns` ORDER BY `id` ASC", "ER_OnMatrunsLoad");
    return 1;
}

forward ER_OnMatrunsLoad();
public ER_OnMatrunsLoad()
{
    new rows = cache_num_rows();
    for(new r; r < rows && MatrunCount < MAX_MATRUNS; r++)
    {
        ER_LoadMatrunRow(r, MatrunCount);
        if(Matruns[MatrunCount][mrEnabled]) ER_CreateMatrunWorld(MatrunCount);
        MatrunCount++;
    }
    printf("[Matruns] Loaded %d material runs.", MatrunCount);
    return 1;
}

stock ER_ReloadMatrunBySQLID(sqlid, playerid = INVALID_PLAYER_ID)
{
    new q[128];
    mysql_format(MainPipeline, q, sizeof(q), "SELECT * FROM `job_matruns` WHERE `id`=%d LIMIT 1", sqlid);
    mysql_tquery(MainPipeline, q, "ER_OnSingleMatrunReload", "ii", sqlid, playerid);
    return 1;
}

forward ER_OnSingleMatrunReload(sqlid, playerid);
public ER_OnSingleMatrunReload(sqlid, playerid)
{
    new rows = cache_num_rows();
    new idx = ER_FindMatrunBySQLID(sqlid);
    ...
    if(!rows)
    {
        ...
        {
            ER_DestroyMatrunWorld(idx);
            Matruns[idx][mrEnabled] = 0;
            Matruns[idx][mrSQLID] = 0;
        }
        if(playerid != INVALID_PLAYER_ID && IsPlayerConnected(playerid)) ER_Send(playerid, COLOR_GREY, "Material run not found.");
        return 1;
    }

    if(idx == -1)
    {
        if(MatrunCount >= MAX_MATRUNS) return 0;
        idx = MatrunCount++;
    }
    else ER_DestroyMatrunWorld(idx);

    ER_LoadMatrunRow(0, idx);
    if(Matruns[idx][mrEnabled]) ER_CreateMatrunWorld(idx);
    if(playerid != INVALID_PLAYER_ID && IsPlayerConnected(playerid)) ER_ShowMatrunEditor(playerid, sqlid);
    return 1;
}


stock ER_FindMatrunBySQLID(id)
{
    for(new i; i < MatrunCount; i++) if(Matruns[i][mrSQLID] == id) return i;
    return -1;
}

stock ER_GetNearestMatrunPickup(playerid)
{
    for(new i; i < MatrunCount; i++)
    {
        if(!Matruns[i][mrEnabled]) continue;
        if(GetPlayerVirtualWorld(playerid) == Matruns[i][mrPickupVW] && GetPlayerInterior(playerid) == Matruns[i][mrPickupInt]
            && IsPlayerInRangeOfPoint(playerid, 5.0, Matruns[i][mrPickupX], Matruns[i][mrPickupY], Matruns[i][mrPickupZ])) return i;
    }
    return -1;
}

stock ER_AddPointIncome(pointid, amount)
{
    if(pointid <= 0 || amount <= 0) return 0;
    new q[192];
    mysql_format(MainPipeline, q, sizeof(q), "UPDATE `points` SET `safe_balance`=`safe_balance`+%d WHERE `id`=%d AND `enabled`=1", amount, pointid);
    mysql_tquery(MainPipeline, q);
    ER_ReloadPointBySQLID(pointid);
    return 1;
}

stock ER_ShowMatrunEditor(playerid, id)
{
    new idx = ER_FindMatrunBySQLID(id);
    if(idx == -1) return ER_Send(playerid, COLOR_GREY, "Invalid material run."), 1;

    SetPVarInt(playerid, "EditingMatrun", id);
    new title[96], body[1024];
    format(title, sizeof(title), "Material Run Editor - ID %d", id);
    format(body, sizeof(body),
        "Admin Name: %s\nPickup Name: %s\nDropoff Name: %s\nPickup Position\nDropoff Position\nPickup Icon: %d\nDropoff Icon: %d\nPackage Amount: %d\nPackage Price: %s\nMaterials Reward: %d\nPickup Point ID: %d\nDropoff Point ID: %d\nFactory Cut: %d%%\nEnabled: %s\nReload Matrun\nDelete",
        Matruns[idx][mrName],
        Matruns[idx][mrPickupName],
        Matruns[idx][mrDropoffName],
        Matruns[idx][mrPickupIcon],
        Matruns[idx][mrDropoffIcon],
        Matruns[idx][mrPackageAmount],
        ER_FormatMoney(Matruns[idx][mrPackageCost]),
        Matruns[idx][mrMaterialReward],
        Matruns[idx][mrPickupPointID],
        Matruns[idx][mrDropoffPointID],
        Matruns[idx][mrDropoffPointCutPercent],
        Matruns[idx][mrEnabled] ? ("Yes") : ("No")
    );
    return ShowPlayerDialog(playerid, DIALOG_MRUN_EDIT, DIALOG_STYLE_LIST, title, body, "Select", "Close");
}

stock ER_ReloadMatrunsForAdmin(playerid = INVALID_PLAYER_ID)
{
    ER_LoadMatruns();
    if(playerid != INVALID_PLAYER_ID && IsPlayerConnected(playerid)) ER_Send(playerid, COLOR_GREEN, "Material runs reloaded.");
    return 1;
}

stock ER_OnMatrunCheckpoint(playerid)
{
    new id = GetPVarInt(playerid, "CarryingMatrunID");
    if(id <= 0) return 0;
    ...
    if(idx == -1)
    {
        ...
        DeletePVar(playerid, "CarryingMatrunID");
        DeletePVar(playerid, "CarryingMatrunPackages");
        return 1;
    }

    if(!IsPlayerInRangeOfPoint(playerid, 7.0, Matruns[idx][mrDropoffX], Matruns[idx][mrDropoffY], Matruns[idx][mrDropoffZ])) return 0;

    DisablePlayerCheckpoint(playerid);
    DeletePVar(playerid, "CarryingMatrunID");
    DeletePVar(playerid, "CarryingMatrunPackages");

    PlayerInfo[playerid][pMaterials] += Matruns[idx][mrMaterialReward];

    // Dropoff/factory point gets a processing cut based on the original package price.
    new factoryIncome = (Matruns[idx][mrPackageCost] * Matruns[idx][mrDropoffPointCutPercent]) / 100;
    if(factoryIncome > 0) ER_AddPointIncome(Matruns[idx][mrDropoffPointID], factoryIncome);

    ER_SaveCharacter(playerid);

    new msg[144];
    format(msg, sizeof(msg), "You delivered the material packages and received %d materials.", Matruns[idx][mrMaterialReward]);
...
CMD:createjob(playerid, params[])
{
    ...
    if(sscanf(params, "s[32]S(Job)[64]", typeText, name))
    {
        ER_Send(playerid, COLOR_GREY, "USAGE: /createjob [type/id] [name]");
        ER_Send(playerid, COLOR_GREY, "Job types: mechanic, taxi, trucker, pizza, arms, drugs, detective, lawyer, bus, garbage, miner, fisher, bodyguard, bartender, craftsman, boxer.");
		return 1;
	}
...
stock ER_JobDialog(playerid, dialogid, response, listitem, const inputtext[])
{
    ...
    if(dialogid == DIALOG_MRUN_LIST)
    {
        if(!response) return 1;
        if(listitem < 0 || listitem >= MatrunCount) return 1;
        return ER_ShowMatrunEditor(playerid, Matruns[listitem][mrSQLID]);
    }
    if(dialogid == DIALOG_MRUN_EDIT)
    {
        new id = GetPVarInt(playerid, "EditingMatrun"), idx = ER_FindMatrunBySQLID(id), q[512];
        if(idx == -1) return ER_Send(playerid, COLOR_GREY, "Invalid material run."), 1;
        if(!response) return 1;
        SetPVarInt(playerid, "MatrunEditAction", listitem);
        switch(listitem)
        {
            case 0: return ShowPlayerDialog(playerid, DIALOG_MRUN_INPUT, DIALOG_STYLE_INPUT, "Matrun Admin Name", "Enter admin/internal material run name:", "Save", "Back");
            case 1: return ShowPlayerDialog(playerid, DIALOG_MRUN_INPUT, DIALOG_STYLE_INPUT, "Pickup Name", "Enter pickup location display name:", "Save", "Back");
            case 2: return ShowPlayerDialog(playerid, DIALOG_MRUN_INPUT, DIALOG_STYLE_INPUT, "Dropoff Name", "Enter dropoff/factory display name:", "Save", "Back");
            case 3:
            {
                new Float:x, Float:y, Float:z;
                GetPlayerPos(playerid, x, y, z);
                mysql_format(MainPipeline, q, sizeof(q), "UPDATE `job_matruns` SET `pickup_x`=%f,`pickup_y`=%f,`pickup_z`=%f,`pickup_int`=%d,`pickup_vw`=%d WHERE `id`=%d", x, y, z, GetPlayerInterior(playerid), GetPlayerVirtualWorld(playerid), id);
                mysql_tquery(MainPipeline, q);
                Matruns[idx][mrPickupX] = x; Matruns[idx][mrPickupY] = y; Matruns[idx][mrPickupZ] = z; Matruns[idx][mrPickupInt] = GetPlayerInterior(playerid); Matruns[idx][mrPickupVW] = GetPlayerVirtualWorld(playerid);
                ER_RefreshMatrunWorld(idx);
                ER_Send(playerid, COLOR_GREEN, "Matrun pickup position saved.");
                return ER_ShowMatrunEditor(playerid, id);
            }
            case 4:
            {
                new Float:x, Float:y, Float:z;
                GetPlayerPos(playerid, x, y, z);
                mysql_format(MainPipeline, q, sizeof(q), "UPDATE `job_matruns` SET `dropoff_x`=%f,`dropoff_y`=%f,`dropoff_z`=%f,`dropoff_int`=%d,`dropoff_vw`=%d WHERE `id`=%d", x, y, z, GetPlayerInterior(playerid), GetPlayerVirtualWorld(playerid), id);
                mysql_tquery(MainPipeline, q);
                Matruns[idx][mrDropoffX] = x; Matruns[idx][mrDropoffY] = y; Matruns[idx][mrDropoffZ] = z; Matruns[idx][mrDropoffInt] = GetPlayerInterior(playerid); Matruns[idx][mrDropoffVW] = GetPlayerVirtualWorld(playerid);
                ER_RefreshMatrunWorld(idx);
                ER_Send(playerid, COLOR_GREEN, "Matrun dropoff position saved.");
                return ER_ShowMatrunEditor(playerid, id);
            }
            case 5: return ShowPlayerDialog(playerid, DIALOG_MRUN_INPUT, DIALOG_STYLE_INPUT, "Pickup Icon", "Enter pickup icon model ID:", "Save", "Back");
            case 6: return ShowPlayerDialog(playerid, DIALOG_MRUN_INPUT, DIALOG_STYLE_INPUT, "Dropoff Icon", "Enter dropoff icon model ID:", "Save", "Back");
            case 7: return ShowPlayerDialog(playerid, DIALOG_MRUN_INPUT, DIALOG_STYLE_INPUT, "Package Amount", "Enter package amount, e.g. 10:", "Save", "Back");
            case 8: return ShowPlayerDialog(playerid, DIALOG_MRUN_INPUT, DIALOG_STYLE_INPUT, "Package Price", "Enter package price:", "Save", "Back");
            case 9: return ShowPlayerDialog(playerid, DIALOG_MRUN_INPUT, DIALOG_STYLE_INPUT, "Materials Reward", "Enter materials rewarded on delivery:", "Save", "Back");
            case 10: return ShowPlayerDialog(playerid, DIALOG_MRUN_INPUT, DIALOG_STYLE_INPUT, "Pickup Point ID", "Enter pickup point ID, or 0 for none:\nThe pickup point gets the full package price.", "Save", "Back");
            case 11: return ShowPlayerDialog(playerid, DIALOG_MRUN_INPUT, DIALOG_STYLE_INPUT, "Dropoff Point ID", "Enter dropoff/factory point ID, or 0 for none:", "Save", "Back");
            case 12: return ShowPlayerDialog(playerid, DIALOG_MRUN_INPUT, DIALOG_STYLE_INPUT, "Factory Cut Percent", "Enter percentage of the package price added to the dropoff/factory point safe:\nExample: 20", "Save", "Back");
            case 13:
            {
                mysql_format(MainPipeline, q, sizeof(q), "UPDATE `job_matruns` SET `enabled`=IF(`enabled`=1,0,1) WHERE `id`=%d", id);
                mysql_tquery(MainPipeline, q);
                Matruns[idx][mrEnabled] = !Matruns[idx][mrEnabled];
                ER_RefreshMatrunWorld(idx);
                ER_Send(playerid, COLOR_GREEN, "Matrun enabled state toggled.");
                return ER_ShowMatrunEditor(playerid, id);
            }
            case 14:
            {
                ER_Send(playerid, COLOR_GREEN, "Material run reloaded.");
                return ER_ReloadMatrunBySQLID(id, playerid);
            }
            case 15: return ShowPlayerDialog(playerid, DIALOG_MRUN_DELETE, DIALOG_STYLE_MSGBOX, "Delete Material Run", "Disable/delete this material run?", "Delete", "Back");
        }
        return 1;
    }
    if(dialogid == DIALOG_MRUN_INPUT)
    {
        new id = GetPVarInt(playerid, "EditingMatrun"), action = GetPVarInt(playerid, "MatrunEditAction"), q[512], value, idx = ER_FindMatrunBySQLID(id);
        if(!response) return ER_ShowMatrunEditor(playerid, id);
        if(idx == -1) return ER_Send(playerid, COLOR_GREY, "Invalid material run."), 1;
        new oldPickupPoint = Matruns[idx][mrPickupPointID], oldDropoffPoint = Matruns[idx][mrDropoffPointID];
        if(action == 0 || action == 1 || action == 2)
        {
            if(strlen(inputtext) < 2 || strlen(inputtext) > 63) return ER_Send(playerid, COLOR_GREY, "Name must be 2-63 characters."), 1;
            if(action == 0) { mysql_format(MainPipeline, q, sizeof(q), "UPDATE `job_matruns` SET `name`='%e' WHERE `id`=%d", inputtext, id); format(Matruns[idx][mrName], 64, "%s", inputtext); }
            else if(action == 1) { mysql_format(MainPipeline, q, sizeof(q), "UPDATE `job_matruns` SET `pickup_name`='%e' WHERE `id`=%d", inputtext, id); format(Matruns[idx][mrPickupName], 64, "%s", inputtext); }
            else { mysql_format(MainPipeline, q, sizeof(q), "UPDATE `job_matruns` SET `dropoff_name`='%e' WHERE `id`=%d", inputtext, id); format(Matruns[idx][mrDropoffName], 64, "%s", inputtext); }
        }
        else
        {
            value = strval(inputtext);
            if(value < 0) return ER_Send(playerid, COLOR_GREY, "Value cannot be negative."), 1;
            switch(action)
            {
                case 5: { mysql_format(MainPipeline, q, sizeof(q), "UPDATE `job_matruns` SET `pickup_icon`=%d WHERE `id`=%d", value, id); Matruns[idx][mrPickupIcon] = value; }
                case 6: { mysql_format(MainPipeline, q, sizeof(q), "UPDATE `job_matruns` SET `dropoff_icon`=%d WHERE `id`=%d", value, id); Matruns[idx][mrDropoffIcon] = value; }
                case 7: { mysql_format(MainPipeline, q, sizeof(q), "UPDATE `job_matruns` SET `package_amount`=%d WHERE `id`=%d", value, id); Matruns[idx][mrPackageAmount] = value; }
                case 8: { mysql_format(MainPipeline, q, sizeof(q), "UPDATE `job_matruns` SET `package_cost`=%d WHERE `id`=%d", value, id); Matruns[idx][mrPackageCost] = value; }
                case 9: { mysql_format(MainPipeline, q, sizeof(q), "UPDATE `job_matruns` SET `material_reward`=%d WHERE `id`=%d", value, id); Matruns[idx][mrMaterialReward] = value; }
                case 10: { mysql_format(MainPipeline, q, sizeof(q), "UPDATE `job_matruns` SET `pickup_point_id`=%d WHERE `id`=%d", value, id); Matruns[idx][mrPickupPointID] = value; }
                case 11: { mysql_format(MainPipeline, q, sizeof(q), "UPDATE `job_matruns` SET `dropoff_point_id`=%d WHERE `id`=%d", value, id); Matruns[idx][mrDropoffPointID] = value; }
                case 12:
                {
                    if(value > 100) value = 100;
                    mysql_format(MainPipeline, q, sizeof(q), "UPDATE `job_matruns` SET `dropoff_point_cut_percent`=%d WHERE `id`=%d", value, id);
                    Matruns[idx][mrDropoffPointCutPercent] = value;
                }
                default: return ER_ShowMatrunEditor(playerid, id);
            }
        }
        mysql_tquery(MainPipeline, q);
        ER_RefreshMatrunWorld(idx);
        if(action == 10)
        {
            if(oldPickupPoint > 0 && oldPickupPoint != Matruns[idx][mrPickupPointID]) ER_ReloadPointBySQLID(oldPickupPoint);
            if(Matruns[idx][mrPickupPointID] > 0) ER_ReloadPointBySQLID(Matruns[idx][mrPickupPointID]);
        }
        else if(action == 11)
        {
            if(oldDropoffPoint > 0 && oldDropoffPoint != Matruns[idx][mrDropoffPointID]) ER_ReloadPointBySQLID(oldDropoffPoint);
            if(Matruns[idx][mrDropoffPointID] > 0) ER_ReloadPointBySQLID(Matruns[idx][mrDropoffPointID]);
        }
        ER_Send(playerid, COLOR_GREEN, "Material run updated.");
        return ER_ShowMatrunEditor(playerid, id);
    }
    if(dialogid == DIALOG_MRUN_DELETE)
    {
        new id = GetPVarInt(playerid, "EditingMatrun"), q[128];
        if(!response) return ER_ShowMatrunEditor(playerid, id);
        mysql_format(MainPipeline, q, sizeof(q), "UPDATE `job_matruns` SET `enabled`=0 WHERE `id`=%d", id);
        mysql_tquery(MainPipeline, q);
        new idx = ER_FindMatrunBySQLID(id); if(idx != -1) { Matruns[idx][mrEnabled] = 0; ER_RefreshMatrunWorld(idx); }
        ER_Send(playerid, COLOR_GREEN, "Material run deleted/disabled."); return 1;
    }
            ...
            default:
            {
                ...
                else if(jt == JOB_TYPE_CRAFTSMAN)
                {
                    switch(listitem)
                    {
                        ...
                        case 6: field = 31;
                        case 7: field = 32;
                        case 8: return ER_ShowMatrunList(playerid);
                    }
                }
                else if(jt == JOB_TYPE_ARMS)
                {
                    if(listitem == 5)
                    {
                        ER_Send(playerid, COLOR_YELLOW, "Weapon unlock editor will use enabled weapons from /editweapons in the next Arms Dealer pass.");
                        return ER_ShowJobTypeEditor(playerid, jt);
                    }
                    if(listitem == 6) return ER_ShowMatrunList(playerid);
                }
                else
                {
...
CMD:creatematrun(playerid, params[])
{
    new name[64];
    if(!ER_IsAdmin(playerid, ADMIN_HEAD)) return ER_Send(playerid, COLOR_GREY, "You are not authorized.");
    if(sscanf(params, "S(Material Run)[64]", name)) return ER_Send(playerid, COLOR_GREY, "USAGE: /creatematrun [name optional]");

    new Float:x, Float:y, Float:z, q[512];
    GetPlayerPos(playerid, x, y, z);
    mysql_format(MainPipeline, q, sizeof(q),
        "INSERT INTO `job_matruns` (`name`,`pickup_name`,`dropoff_name`,`pickup_x`,`pickup_y`,`pickup_z`,`pickup_int`,`pickup_vw`,`dropoff_x`,`dropoff_y`,`dropoff_z`,`dropoff_int`,`dropoff_vw`,`package_amount`,`package_cost`,`material_reward`,`pickup_point_id`,`dropoff_point_id`,`dropoff_point_cut_percent`,`pickup_icon`,`dropoff_icon`,`enabled`) VALUES ('%e','Material Pickup','Material Factory',%f,%f,%f,%d,%d,%f,%f,%f,%d,%d,10,75,250,0,0,20,1271,1239,1)",
        name, x, y, z, GetPlayerInterior(playerid), GetPlayerVirtualWorld(playerid), x + 5.0, y, z, GetPlayerInterior(playerid), GetPlayerVirtualWorld(playerid));
    mysql_tquery(MainPipeline, q, "ER_OnMatrunCreated", "i", playerid);
    return 1;
}

forward ER_OnMatrunCreated(playerid);
public ER_OnMatrunCreated(playerid)
{
    new id = cache_insert_id(), msg[96];
    format(msg, sizeof(msg), "Material run created with ID %d.", id);
    ER_Send(playerid, COLOR_GREEN, msg);
    return ER_ReloadMatrunBySQLID(id, playerid);
}


stock ER_ShowMatrunList(playerid)
{
    new list[2048], line[128];
    for(new i; i < MatrunCount; i++)
    {
        format(line, sizeof(line), "%d - %s - Packages: %d - Reward: %d mats\n", Matruns[i][mrSQLID], Matruns[i][mrName], Matruns[i][mrPackageAmount], Matruns[i][mrMaterialReward]);
        strcat(list, line, sizeof(list));
    }
    if(!list[0]) format(list, sizeof(list), "No material runs created.");
    return ShowPlayerDialog(playerid, DIALOG_MRUN_LIST, DIALOG_STYLE_LIST, "Material Runs", list, "Edit", "Close");
}

CMD:editmatruns(playerid, params[])
{
    #pragma unused params
    if(!ER_IsAdmin(playerid, ADMIN_HEAD)) return ER_Send(playerid, COLOR_GREY, "You are not authorized.");
    return ER_ShowMatrunList(playerid);
}

CMD:editmatrun(playerid, params[])
{
    new id;
    if(!ER_IsAdmin(playerid, ADMIN_HEAD)) return ER_Send(playerid, COLOR_GREY, "You are not authorized.");
    if(sscanf(params, "d", id)) return ER_ShowMatrunList(playerid);
    return ER_ShowMatrunEditor(playerid, id);
}

CMD:deletematrun(playerid, params[])
{
    new id, q[128];
    if(!ER_IsAdmin(playerid, ADMIN_HEAD)) return ER_Send(playerid, COLOR_GREY, "You are not authorized.");
    if(sscanf(params, "d", id)) return ER_Send(playerid, COLOR_GREY, "USAGE: /deletematrun [id]");
    mysql_format(MainPipeline, q, sizeof(q), "UPDATE `job_matruns` SET `enabled`=0 WHERE `id`=%d", id);
    mysql_tquery(MainPipeline, q);
    ER_ReloadMatrunBySQLID(id, playerid);
    return ER_Send(playerid, COLOR_GREEN, "Material run deleted/disabled.");
}

CMD:reloadmatruns(playerid, params[])
{
    #pragma unused params
    if(!ER_IsAdmin(playerid, ADMIN_HEAD)) return ER_Send(playerid, COLOR_GREY, "You are not authorized.");
    return ER_ReloadMatrunsForAdmin(playerid);
}

CMD:getmats(playerid, params[])
{
    #pragma unused params
    if(!ER_PlayerHasJobType(playerid, JOB_TYPE_ARMS) && !ER_PlayerHasJobType(playerid, JOB_TYPE_CRAFTSMAN))
        return ER_Send(playerid, COLOR_GREY, "You are not an Arms Dealer or Craftsman."), 1;

    if(GetPVarInt(playerid, "CarryingMatrunID") > 0)
        return ER_Send(playerid, COLOR_GREY, "You are already carrying material packages."), 1;

    new idx = ER_GetNearestMatrunPickup(playerid);
    if(idx == -1) return ER_Send(playerid, COLOR_GREY, "You are not at a material pickup point."), 1;

    new cost = Matruns[idx][mrPackageCost];
    if(cost > 0 && PlayerInfo[playerid][pCash] < cost)
        return ER_Send(playerid, COLOR_GREY, "You do not have enough cash for the material packages."), 1;
    ...
    // Pickup point gets the full package price because the money is paid at pickup.
    if(cost > 0) ER_AddPointIncome(Matruns[idx][mrPickupPointID], cost);

    SetPVarInt(playerid, "CarryingMatrunID", Matruns[idx][mrSQLID]);
    SetPVarInt(playerid, "CarryingMatrunPackages", Matruns[idx][mrPackageAmount]);
    SetPlayerCheckpoint(playerid, Matruns[idx][mrDropoffX], Matruns[idx][mrDropoffY], Matruns[idx][mrDropoffZ], 5.0);
    ER_SaveCharacter(playerid);

    new msg[160];
    format(msg, sizeof(msg), "You received %d material packages for %s. Deliver them to %s.", Matruns[idx][mrPackageAmount], ER_FormatMoney(cost), Matruns[idx][mrDropoffName]);
...
CMD:repair(playerid, params[])
{
    ...
    if(!ER_IsHoodOpen(vehicleid)) return ER_Send(playerid, COLOR_GREY, "The vehicle hood must be opened first.");
...
stock ER_AcceptRepairOffer(playerid)
{
    ...
    if(!ER_IsHoodOpen(vehicleid)) return ER_Send(playerid, COLOR_GREY, "The vehicle hood must stay open.");
