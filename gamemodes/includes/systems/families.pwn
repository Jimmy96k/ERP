...
stock ER_CountOnlineFamilyMembers(fid)
{
    new count;
    foreach(new i : Player) if(PlayerInfo[i][pLoggedIn] && PlayerInfo[i][pFamily] == fid) count++;
    return count;
}
...
CMD:families(playerid, params[])
{
    ...
    {
        new body[4096], line[256], hex[8], leader[MAX_PLAYER_NAME_EX];
        format(body, sizeof(body), "{F6A800}____________________ Families ____________________\n");
        for(new i; i < FamilyCount; i++)
        {
            FamilyListSQL[playerid][i+1] = Families[i][fSQLID];
            ER_ColorToDialogHex(Families[i][fColor], hex, sizeof(hex));
            if(Families[i][fLeaderID] > 0 && strcmp(Families[i][fLeaderName], "Nobody", true)) format(leader, sizeof(leader), "%s", Families[i][fLeaderName]);
            else format(leader, sizeof(leader), "None");
            format(line, sizeof(line), "{F6A800}Name: {%s}%s {F6A800}| Leader: {FFFFFF}%s {F6A800}| Members: {FFFFFF}%d {F6A800}| Online: {FFFFFF}%d\n",
                hex, Families[i][fName], leader, Families[i][fMembers], ER_CountOnlineFamilyMembers(Families[i][fSQLID]));
            strcat(body, line, sizeof(body));
        }
        return ShowPlayerDialog(playerid, DIALOG_FAMILIES_VIEW, DIALOG_STYLE_MSGBOX, "Families", body, "Close", "");
    }
...
{
    SetPVarInt(playerid, "EditingFamily", fid);
    new idx = ER_FindFamilyIndexBySQLID(fid), list[1400], nameColor[32], famColor[32], crewColor[32], leader[MAX_PLAYER_NAME_EX], status[16], permText[16];
    if(idx == -1) return ER_Send(playerid, COLOR_GREY, "Invalid family."), 1;
    ER_GetNamedColorDisplayByValue(Families[idx][fColor], nameColor, sizeof(nameColor));
    ...
    format(list, sizeof(list), "Name: %s\nLeader: %s\nMOTD: %s\nFamily Color: %s\nFamily Chat Color: %s\nCrew Chat Color: %s\nRanks: 6 Ranks\nCrews: 3 Crews\nLockers\nSafes\nPermissions: %s\nStatus: %s\nReload This Family",
        Families[idx][fName], leader, Families[idx][fMOTD], nameColor, famColor, crewColor, permText, status);
...
stock ER_FamilyDialog(playerid, dialogid, response, listitem, inputtext[])
{
    ...
    if(dialogid == DIALOG_FAMILY_EDITOR)
    {
        ...
        switch(listitem)
        {
            ...
            case 3: { SetPVarInt(playerid, "FamilyColorField", 0); ER_ShowNamedColorDialog(playerid, DIALOG_FAMILY_COLOR_SELECT, "Family Color"); }
            case 4: { SetPVarInt(playerid, "FamilyColorField", 1); ER_ShowNamedColorDialog(playerid, DIALOG_FAMILY_COLOR_SELECT, "Family Chat Color"); }
            case 5: { SetPVarInt(playerid, "FamilyColorField", 2); ER_ShowNamedColorDialog(playerid, DIALOG_FAMILY_COLOR_SELECT, "Crew Chat Color"); }
            case 6: return ER_ShowFamilyRanksEditor(playerid, GetPVarInt(playerid, "EditingFamily"));
            case 7: return ER_ShowFamilyCrewsEditor(playerid, GetPVarInt(playerid, "EditingFamily"));
            case 8: return ER_ShowFamilyLockerEditor(playerid, GetPVarInt(playerid, "EditingFamily"));
            case 9: return ER_ShowFamilyLockerEditor(playerid, GetPVarInt(playerid, "EditingFamily"));
            case 10: return ER_ShowFamilyPermsEditor(playerid, GetPVarInt(playerid, "EditingFamily"));
            case 11: { SetPVarInt(playerid, "FamilyInputField", 50); ShowPlayerDialog(playerid, DIALOG_FAMILY_INPUT, DIALOG_STYLE_LIST, "Family Status", "Enabled\nDisabled", "Save", "Back"); }
            case 12: { ER_LoadFamilies(); ER_Send(playerid, COLOR_GREEN, "Families reloaded."); }
        }
        return 1;
    }
    ...
    if(dialogid == DIALOG_FAMILY_COLOR_SELECT)
    {
        ...
        if(field == 0) mysql_format(MainPipeline, q, sizeof(q), "UPDATE `families` SET `color`=%d WHERE `id`=%d", color, fid);
        else if(field == 1) mysql_format(MainPipeline, q, sizeof(q), "UPDATE `families` SET `radio_color`=%d WHERE `id`=%d", color, fid);
