...
#if !defined MAX_HOUSE_GUNLOCKER_SLOTS
    #define MAX_HOUSE_GUNLOCKER_SLOTS 40
#endif
new HouseGunList[MAX_PLAYERS][MAX_HOUSE_GUNLOCKER_SLOTS];
...
stock ER_FormatHouseStorageList(idx, playerid, list[], size)
{
    new gunCount = ER_CountHouseGuns(Houses[idx][hSQLID]);
    format(list, size, "Cash Safe: %s (You have: %s)\nMaterials: %d (You have: %d)\nPot: %d (You have: %d)\nCrack: %d (You have: %d)\nGun Locker (%d stored)",
        ER_FormatMoney(Houses[idx][hSafeBalance]), ER_FormatMoney(PlayerInfo[playerid][pCash]),
        Houses[idx][hMaterials], PlayerInfo[playerid][pMaterials],
        Houses[idx][hPot], PlayerInfo[playerid][pPot],
        Houses[idx][hCrack], PlayerInfo[playerid][pCrack],
        gunCount);
    return 1;
}
...
stock ER_ShowHouseStorage(playerid)
{
    ...
    SetPVarInt(playerid, "HStorageHouse", Houses[idx][hSQLID]);
    new list[400];
    ER_FormatHouseStorageList(idx, playerid, list, sizeof(list));
    return ShowPlayerDialog(playerid, DIALOG_HOUSE_STORAGE, DIALOG_STYLE_LIST, "House Storage", list, "Select", "Close");
}

// Returns the resource name for titles/messages (0=Cash,1=Materials,2=Pot,3=Crack).
stock ER_GetHouseStorageResourceName(resource, dest[], size)
{
    switch(resource)
    {
        case 0: format(dest, size, "Cash");
        case 1: format(dest, size, "Materials");
        case 2: format(dest, size, "Pot");
        case 3: format(dest, size, "Crack");
        default: format(dest, size, "Unknown");
    }
    return 1;
}

// Moves `amount` of the given resource (0=Cash,1=Materials,2=Pot,3=Crack)
// between the player and the house, in the direction given by action
// (1 = deposit player->house, 2 = withdraw house->player).
stock ER_ProcessHouseStorageAction(playerid, idx, resource, action, amount)
{
    new hid = Houses[idx][hSQLID], q[160], column[16], playerHas, houseHas;
    switch(resource)
    {
        case 0: { format(column, sizeof(column), "safe_balance"); playerHas = PlayerInfo[playerid][pCash]; houseHas = Houses[idx][hSafeBalance]; }
        case 1: { format(column, sizeof(column), "materials"); playerHas = PlayerInfo[playerid][pMaterials]; houseHas = Houses[idx][hMaterials]; }
        case 2: { format(column, sizeof(column), "pot"); playerHas = PlayerInfo[playerid][pPot]; houseHas = Houses[idx][hPot]; }
        case 3: { format(column, sizeof(column), "crack"); playerHas = PlayerInfo[playerid][pCrack]; houseHas = Houses[idx][hCrack]; }
        default: return 0;
    }

    if(action == 1)
    {
        if(playerHas < amount) return ER_Send(playerid, COLOR_GREY, "You do not have that much."), 0;
        playerHas -= amount;
        houseHas += amount;
    }
    else
    {
        if(houseHas < amount) return ER_Send(playerid, COLOR_GREY, "The house does not have that much stored."), 0;
        houseHas -= amount;
        playerHas += amount;
    }
    ...
    switch(resource)
    {
        case 0:
        {
            Houses[idx][hSafeBalance] = houseHas;
            PlayerInfo[playerid][pCash] = playerHas;
            GivePlayerMoney(playerid, (action == 1) ? -amount : amount);
        }
        case 1: { Houses[idx][hMaterials] = houseHas; PlayerInfo[playerid][pMaterials] = playerHas; }
        case 2: { Houses[idx][hPot] = houseHas; PlayerInfo[playerid][pPot] = playerHas; }
        case 3: { Houses[idx][hCrack] = houseHas; PlayerInfo[playerid][pCrack] = playerHas; }
    }

    mysql_format(MainPipeline, q, sizeof(q), "UPDATE `houses` SET `%s`=%d WHERE `id`=%d", column, houseHas, hid);
...
// -----------------------------------------------------------------------------
// House Gun Locker
// Real weapon storage: deposit takes the physical weapon + its current ammo
// out of the player's hands, withdraw hands it back exactly as stored.
// -----------------------------------------------------------------------------
stock ER_CountHouseGuns(hid)
{
    new q[128]; mysql_format(MainPipeline, q, sizeof(q), "SELECT COUNT(*) AS `total` FROM `house_weapons` WHERE `house_id`=%d", hid);
    new Cache:res = mysql_query(MainPipeline, q);
    new rows, total; cache_get_row_count(rows);
    if(rows > 0) cache_get_value_name_int(0, "total", total);
    cache_delete(res);
    return total;
}

stock ER_ShowHouseGunLocker(playerid, idx)
{
    if(idx < 0 || idx >= HouseCount) return ER_Send(playerid, COLOR_GREY, "Invalid house.");
    SetPVarInt(playerid, "GunLockerHouse", Houses[idx][hSQLID]);
    new q[160];
    mysql_format(MainPipeline, q, sizeof(q), "SELECT * FROM `house_weapons` WHERE `house_id`=%d ORDER BY `id` ASC LIMIT %d", Houses[idx][hSQLID], MAX_HOUSE_GUNLOCKER_SLOTS);
    new Cache:res = mysql_query(MainPipeline, q);
    new rows; cache_get_row_count(rows);
    new list[2048], line[96], wname[32], weaponid, ammo, gunid;
    for(new i; i < rows && i < MAX_HOUSE_GUNLOCKER_SLOTS; i++)
    {
        cache_get_value_name_int(i, "id", gunid);
        cache_get_value_name_int(i, "weaponid", weaponid);
        cache_get_value_name_int(i, "ammo", ammo);
        HouseGunList[playerid][i] = gunid;
        GetWeaponName(weaponid, wname, sizeof(wname));
        format(line, sizeof(line), "%s (Ammo: %d)\n", wname, ammo);
        strcat(list, line, sizeof(list));
    }
    new count = (rows < MAX_HOUSE_GUNLOCKER_SLOTS) ? rows : MAX_HOUSE_GUNLOCKER_SLOTS;
    SetPVarInt(playerid, "GunLockerCount", count);
    cache_delete(res);
    if(!count) return ER_Send(playerid, COLOR_GREY, "This house's gun locker is empty."), 1;
    return ShowPlayerDialog(playerid, DIALOG_HOUSE_GUNLOCKER, DIALOG_STYLE_LIST, "House Gun Locker", list, "Withdraw", "Close");
}

stock ER_DepositGunCmd(playerid, params[])
{
    new idx = ER_GetNearestHouse(playerid, true);
    if(idx == -1) return ER_Send(playerid, COLOR_GREY, "You are not inside a house.");
    if(!ER_PlayerOwnsHouse(playerid, idx)) return ER_Send(playerid, COLOR_GREY, "You do not own this house.");

    new weaponname[32];
    if(sscanf(params, "s[32]", weaponname)) return ER_Send(playerid, COLOR_GREY, "USAGE: /depositgun [weapon name or id]");

    new slot = ER_FindPlayerWeaponSlot(playerid, weaponname);
    if(slot == -1) return ER_Send(playerid, COLOR_GREY, "You are not carrying that weapon.");

    new weaponid, ammo;
    GetPlayerWeaponData(playerid, slot, weaponid, ammo);
    if(weaponid <= 0) return ER_Send(playerid, COLOR_GREY, "Invalid weapon.");

    if(ER_CountHouseGuns(Houses[idx][hSQLID]) >= MAX_HOUSE_GUNLOCKER_SLOTS) return ER_Send(playerid, COLOR_GREY, "This house's gun locker is full.");

    new q[160];
    mysql_format(MainPipeline, q, sizeof(q), "INSERT INTO `house_weapons` (`house_id`,`weaponid`,`ammo`) VALUES (%d,%d,%d)", Houses[idx][hSQLID], weaponid, ammo);
    mysql_tquery(MainPipeline, q);

    ER_RemovePlayerWeaponSlot(playerid, slot);
    ER_SaveCurrentWeapons(playerid);

    new wname[32], msg[96];
    GetWeaponName(weaponid, wname, sizeof(wname));
    format(msg, sizeof(msg), "You store your %s (Ammo: %d) in the house gun locker.", wname, ammo);
    return ER_Send(playerid, COLOR_GREEN, msg);
}
CMD:depositgun(playerid, params[]) { return ER_DepositGunCmd(playerid, params); }
CMD:gundeposit(playerid, params[]) { return ER_DepositGunCmd(playerid, params); }

stock ER_WithdrawGunCmd(playerid, params[])
{
    #pragma unused params
    new idx = ER_GetNearestHouse(playerid, true);
    if(idx == -1) return ER_Send(playerid, COLOR_GREY, "You are not inside a house.");
    if(!ER_PlayerOwnsHouse(playerid, idx)) return ER_Send(playerid, COLOR_GREY, "You do not own this house.");
    return ER_ShowHouseGunLocker(playerid, idx);
}
CMD:withdrawgun(playerid, params[]) { return ER_WithdrawGunCmd(playerid, params); }
CMD:gunwithdraw(playerid, params[]) { return ER_WithdrawGunCmd(playerid, params); }
...
stock ER_HouseDialog(playerid, dialogid, response, listitem, const inputtext[])
{
    ...
    if(dialogid == DIALOG_MY_HOUSE_MENU)
    {
        ...
        switch(listitem)
        {
            ...
            case 4: return ER_ShowHouseGunLocker(playerid, idx);
            case 5: { PlayerInfo[playerid][pSpawnX]=Houses[idx][hIntX]; PlayerInfo[playerid][pSpawnY]=Houses[idx][hIntY]; PlayerInfo[playerid][pSpawnZ]=Houses[idx][hIntZ]; PlayerInfo[playerid][pSpawnA]=Houses[idx][hIntA]; PlayerInfo[playerid][pSpawnInt]=Houses[idx][hIntInt]; PlayerInfo[playerid][pSpawnVW]=Houses[idx][hIntVW]; ER_SaveLastPosition(playerid,true); return ER_Send(playerid, COLOR_GREEN, "House spawn set to this house interior."); }
            case 6:
            {
    ...
    if(dialogid == DIALOG_HOUSE_GUNLOCKER)
    {
        if(!response) return 1;
        new count = GetPVarInt(playerid, "GunLockerCount");
        if(listitem < 0 || listitem >= count) return 1;

        new hid = GetPVarInt(playerid, "GunLockerHouse"), idx = ER_FindHouseIndexBySQLID(hid);
        if(idx == -1 || !ER_PlayerOwnsHouse(playerid, idx)) return ER_Send(playerid, COLOR_GREY, "You no longer have access to this gun locker.");

        new gunid = HouseGunList[playerid][listitem], q[160];
        mysql_format(MainPipeline, q, sizeof(q), "SELECT * FROM `house_weapons` WHERE `id`=%d AND `house_id`=%d LIMIT 1", gunid, hid);
        new Cache:res = mysql_query(MainPipeline, q);
        new rows; cache_get_row_count(rows);
        if(rows <= 0) { cache_delete(res); return ER_Send(playerid, COLOR_GREY, "That weapon is no longer in the locker."); }
        ...
        mysql_format(MainPipeline, q, sizeof(q), "DELETE FROM `house_weapons` WHERE `id`=%d LIMIT 1", gunid);
        mysql_tquery(MainPipeline, q);
        ...
        new wname[32], msg[96];
        GetWeaponName(weaponid, wname, sizeof(wname));
        format(msg, sizeof(msg), "You withdraw your %s (Ammo: %d) from the house gun locker.", wname, ammo);
        return ER_Send(playerid, COLOR_GREEN, msg);
    }
    if(dialogid == DIALOG_HOUSE_STORAGE)
    {
        if(!response) return 1;
        new hid = GetPVarInt(playerid, "HStorageHouse"), idx = ER_FindHouseIndexBySQLID(hid);
        if(idx == -1 || !ER_PlayerOwnsHouse(playerid, idx)) return ER_Send(playerid, COLOR_GREY, "You no longer have access to this house storage.");
        if(listitem == 4) return ER_ShowHouseGunLocker(playerid, idx);
        if(listitem < 0 || listitem > 3) return 1;
        SetPVarInt(playerid, "HStorageResource", listitem);
        new resName[16], title[32];
        ER_GetHouseStorageResourceName(listitem, resName, sizeof(resName));
        format(title, sizeof(title), "%s Storage", resName);
        return ShowPlayerDialog(playerid, DIALOG_HOUSE_STORAGE_RESOURCE, DIALOG_STYLE_LIST, title, "Deposit\nWithdraw", "Select", "Back");
    }
    if(dialogid == DIALOG_HOUSE_STORAGE_RESOURCE)
    {
        if(!response) return ER_ShowHouseStorage(playerid);
        new hid = GetPVarInt(playerid, "HStorageHouse"), idx = ER_FindHouseIndexBySQLID(hid);
        if(idx == -1 || !ER_PlayerOwnsHouse(playerid, idx)) return ER_Send(playerid, COLOR_GREY, "You no longer have access to this house storage.");
        if(listitem != 0 && listitem != 1) return ER_ShowHouseStorage(playerid);
        SetPVarInt(playerid, "HStorageAction", listitem + 1); // 1 = Deposit, 2 = Withdraw

        new resource = GetPVarInt(playerid, "HStorageResource"), resName[16], title[48], body[64];
        ER_GetHouseStorageResourceName(resource, resName, sizeof(resName));
        format(title, sizeof(title), "%s %s", (listitem == 0) ? ("Deposit") : ("Withdraw"), resName);
        format(body, sizeof(body), "Enter the amount of %s to %s:", resName, (listitem == 0) ? ("deposit") : ("withdraw"));
        return ShowPlayerDialog(playerid, DIALOG_HOUSE_STORAGE_INPUT, DIALOG_STYLE_INPUT, title, body, "Confirm", "Back");
    }
    if(dialogid == DIALOG_HOUSE_STORAGE_INPUT)
    {
        if(!response) return ER_ShowHouseStorage(playerid);
        new hid = GetPVarInt(playerid, "HStorageHouse"), idx = ER_FindHouseIndexBySQLID(hid);
        if(idx == -1 || !ER_PlayerOwnsHouse(playerid, idx)) return ER_Send(playerid, COLOR_GREY, "You no longer have access to this house storage.");
        ...
        if(amount <= 0) { ER_Send(playerid, COLOR_GREY, "Enter a valid positive amount."); return ER_ShowHouseStorage(playerid); }

        new resource = GetPVarInt(playerid, "HStorageResource"), action = GetPVarInt(playerid, "HStorageAction");
        if(ER_ProcessHouseStorageAction(playerid, idx, resource, action, amount))
        {
            new resName[16], msg[96];
            ER_GetHouseStorageResourceName(resource, resName, sizeof(resName));
            format(msg, sizeof(msg), "%s successfully %s.", resName, (action == 1) ? ("deposited") : ("withdrawn"));
