...
new bool:ER_PlayerHelmet[MAX_PLAYERS];
...
public ER_OnShowVehicleEditor(playerid)
{
	...
	format(line, sizeof(line), "Fuel Consumption: %.2f gal/km", idx != -1 ? VehicleInfo[idx][vFuelConsumptionRate] : ServerCore[scDefaultVehicleFuelConsumption]);
...
stock ER_VehicleDialog(playerid, dialogid, response, listitem, const inputtext[])
{
	...
	if(dialogid == DIALOG_EDIT_VEHICLE_MENU)
	{
		...
		switch(action)
		{
			...
			case 16:
			{
				new idx = ER_FindVehicleBySQLID(sqlid), msg[160];
				if(idx == -1) return ER_ShowVehicleEditor(playerid, sqlid);
				format(msg, sizeof(msg), "Enter fuel consumption in gallons per KM.\n\nCurrent: %.2f gal/km\nDefault: %.2f gal/km", VehicleInfo[idx][vFuelConsumptionRate], ServerCore[scDefaultVehicleFuelConsumption]);
	...
	if(dialogid == DIALOG_EDIT_VEH_CONSUMPTION)
	{
		...
		if(rate <= 0.0 || rate > 25.0) return ER_Send(playerid, COLOR_GREY, "Fuel consumption must be between 0.01 and 25.00 gal/km.");
...
stock ER_IsBikeOrBuggyModel(model)
{
    switch(model)
    {
        case 448, 461, 462, 463, 468, 471, 481, 509, 510, 521, 522, 523, 581, 586, 571, 572: return 1;
    }
    return 0;
}

stock ER_IsBikeOrBuggyVehicle(vehicleid)
{
    return ER_IsBikeOrBuggyModel(GetVehicleModel(vehicleid));
}
...
stock ER_UpdateVehicleHUD(playerid)
{
    ...
    new vehicleid = GetPlayerVehicleID(playerid);
    new idx = ER_FindVehicleBySpawnID(vehicleid);
    new isTemp = TempVehicle[vehicleid];
    if(idx == -1 && !isTemp)
    {
    ...
    new isBike = ER_IsBikeOrBuggyModel(model);
    ...
    if(isTemp || (idx != -1 && VehicleInfo[idx][vUnlimitedFuel])) format(line, sizeof(line), "~b~Fuel: ~r~U");
    else format(line, sizeof(line), "~b~Fuel: ~w~%d%%", floatround(VehicleInfo[idx][vFuel]));
    PlayerTextDrawSetString(playerid, ER_VehHudFuel[playerid], line);

    if(isTemp) format(line, sizeof(line), "~b~Mileage: ~w~0.0 KM");
    else format(line, sizeof(line), "~b~Mileage: ~w~%.1f KM", VehicleInfo[idx][vMileage]);
    PlayerTextDrawSetString(playerid, ER_VehHudMileage[playerid], line);

    if(isBike) format(line, sizeof(line), "~b~Helmet: %s", ER_PlayerHelmet[playerid] ? "~g~On" : "~r~Off");
...
stock ER_ShowVehicleHUD(playerid)
{
    ...
    new vehicleid = GetPlayerVehicleID(playerid);
    new idx = ER_FindVehicleBySpawnID(vehicleid);
    if(idx == -1 && !TempVehicle[vehicleid]) return 0;
    ...
    if(!ER_IsBikeOrBuggyVehicle(vehicleid))
    {
        PlayerTextDrawShow(playerid, ER_VehHudHood[playerid]);
        PlayerTextDrawShow(playerid, ER_VehHudTrunk[playerid]);
    }
...
stock ER_OnVehicleHudSeatExited(playerid, vehicleid)
{
    ...
    ER_PlayerHelmet[playerid] = false;
    return ER_HideVehicleHUD(playerid);
}
...
public ER_VehicleHudTick()
{
    foreach(new i : Player)
    {
        ...
        new idx = ER_FindVehicleBySpawnID(vehicleid);
        if(idx == -1 && !TempVehicle[vehicleid])
        {
            ER_HideVehicleHUD(i);
            continue;
        }
        ...
        if(ER_VehHudLastPosValid[i])
        {
            ...
            // Add driven distance as kilometers and consume fuel by distance.
            // Temporary /aveh vehicles show HUD but do not save mileage or consume fuel.
            if(dist > 0.50 && dist < 150.0)
            {
                new engine, lights, alarm, doors, bonnet, boot, objective;
                GetVehicleParamsEx(vehicleid, engine, lights, alarm, doors, bonnet, boot, objective);
                ER_NormalizeVehicleParams(engine, lights, alarm, doors, bonnet, boot, objective);
                if(engine == VEHICLE_PARAMS_ON && idx != -1)
                {
                    new Float:distKm = dist / 1000.0;
                    VehicleInfo[idx][vMileage] += distKm;
                    if(!VehicleInfo[idx][vUnlimitedFuel])
                    {
                        ...
                        VehicleInfo[idx][vFuel] -= (distKm * rate);
                        if(VehicleInfo[idx][vFuel] <= 0.0)
                        {
                            VehicleInfo[idx][vFuel] = 0.0;
...
public ER_VehicleFuelTick()
{
    foreach(new i : Player)
    {
        ...
        if(engine == VEHICLE_PARAMS_ON && !VehicleInfo[idx][vUnlimitedFuel])
        {
            if(VehicleInfo[idx][vFuel] <= 0.0)
            {
                VehicleInfo[idx][vFuel] = 0.0;
            ...
            {
                new Float:speed = ER_GetVehicleSpeedKMH(vehicleid);
                if(speed < 1.0)
                {
                    VehicleInfo[idx][vFuel] -= ServerCore[scVehicleIdleFuelGallonsPerTick];
                    if(VehicleInfo[idx][vFuel] < 0.0) VehicleInfo[idx][vFuel] = 0.0;
                }
            }
        }
        ER_SaveVehicleMileage(idx);
        ER_SetVehicleFuel(idx, VehicleInfo[idx][vFuel]);
        if(ER_VehHudVisible[i]) ER_UpdateVehicleHUD(i);
    }
    return 1;
}
...
{
    #pragma unused params
    if(GetPlayerState(playerid) != PLAYER_STATE_DRIVER) return ER_Send(playerid, COLOR_GREY, "You must be driving a vehicle to use this command.");
    new vehicleid = GetPlayerVehicleID(playerid);
    if(ER_IsBikeOrBuggyVehicle(vehicleid))
    {
        ER_PlayerHelmet[playerid] = !ER_PlayerHelmet[playerid];
        ER_SendVehicleActionLine(playerid, vehicleid, ER_PlayerHelmet[playerid] ? "puts on a helmet while on" : "removes their helmet while on");
    }
    else
    {
        ER_PlayerSeatbelt[playerid] = !ER_PlayerSeatbelt[playerid];
        ER_SendVehicleActionLine(playerid, vehicleid, ER_PlayerSeatbelt[playerid] ? "fastens their seatbelt in" : "unfastens their seatbelt in");
    }
    ER_UpdateVehicleHUD(playerid);
    return 1;
}
...
CMD:helmet(playerid, params[])
{
    ...
    if(!ER_IsBikeOrBuggyVehicle(GetPlayerVehicleID(playerid))) return ER_Send(playerid, COLOR_GREY, "This vehicle does not require a helmet.");
    return pc_cmd_seatbelt(playerid, params);
}
...
CMD:vehhud(playerid, params[])
{
    ...
    if(ER_FindVehicleBySpawnID(GetPlayerVehicleID(playerid)) == -1 && !TempVehicle[GetPlayerVehicleID(playerid)]) return ER_Send(playerid, COLOR_GREY, "This vehicle has no HUD data.");
    ER_ShowVehicleHUD(playerid);
