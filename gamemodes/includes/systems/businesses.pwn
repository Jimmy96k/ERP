...
public ER_OnBusinessesLoad()
{
    ...
    for(new r; r < rows && BusinessCount < MAX_BUSINESSES; r++)
    {
        ...
        if(Businesses[BusinessCount][bEnterable] == 0 && Businesses[BusinessCount][bType] != BUSINESS_TYPE_GAS && Businesses[BusinessCount][bType] != BUSINESS_TYPE_DEALERSHIP) Businesses[BusinessCount][bEnterable] = 1;
...
CMD:createbusiness(playerid, params[])
{
    ...
    new q[768];
    mysql_format(MainPipeline, q, sizeof(q), "INSERT INTO `businesses` (`name`,`type`,`owner_type`,`owner_id`,`owner_name`,`price`,`price_mode`,`materials`,`materials_capacity`,`safe_balance`,`ext_x`,`ext_y`,`ext_z`,`ext_a`,`ext_int`,`ext_vw`,`pickup_model`,`pickup_type`,`locked`,`lockable`,`enterable`,`custom_ext`,`custom_int`,`enabled`) VALUES ('%e',%d,0,0,'Nobody',%d,0,0,2000,0,%f,%f,%f,%f,%d,%d,%d,23,0,%d,%d,0,0,1)", typeName, type, price, x, y, z, a, GetPlayerInterior(playerid), GetPlayerVirtualWorld(playerid), pickup, (type == BUSINESS_TYPE_GAS || type == BUSINESS_TYPE_DEALERSHIP) ? 0 : 1,
            (type == BUSINESS_TYPE_GAS || type == BUSINESS_TYPE_DEALERSHIP) ? 0 : 1);
