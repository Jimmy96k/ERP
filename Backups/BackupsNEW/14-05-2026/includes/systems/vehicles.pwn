...
public ER_OnShowVehicleEditor(playerid)
{
	...
	new sqlid, model, c1, c2, paintjob, nos, unlimitednos, hydraulics, wheels, ownerpid, familyid, factionid, spawnid, ownername[24], owner[96], list[1800];
	new row, line[128];
...
stock ER_SendVehicleActionLine(playerid, vehicleid, const action[])
{
    ...
    new Float:x, Float:y, Float:z, msg[160];
    GetPlayerPos(playerid, x, y, z);
    format(msg, sizeof(msg), "* %s %s their %s.", ER_GetName(playerid), action, ER_GetVehicleModelName(VehicleInfo[idx][vModel]));
    ER_NearbyMessage(x, y, z, CHAT_RANGE_NORMAL, COLOR_ME, msg, GetPlayerVirtualWorld(playerid), GetPlayerInterior(playerid));
    return 1;
}
...
stock ER_HandleVehicleCommand(playerid, params[])
{
    ...
    {
        if(!ServerCore[scAllowVehicleEngineWithoutKeys] && !ER_PlayerCanUseVehicle(playerid, vehicleid)) return ER_Send(playerid, COLOR_GREY, "You do not have the keys to this vehicle.");
        SetPVarInt(playerid, "IgnitionVehicle", vehicleid);
        SetPVarInt(playerid, "IgnitionTarget", engine == VEHICLE_PARAMS_ON ? 0 : 1);
        ER_SendVehicleActionLine(playerid, vehicleid, engine == VEHICLE_PARAMS_ON ? "turns off the engine of" : "turns the key in the ignition of");
    ...
    if(!strcmp(option, "lights", true))
    {
        ...
        return ER_Send(playerid, COLOR_GREEN, lights == VEHICLE_PARAMS_ON ? "Vehicle lights turned on." : "Vehicle lights turned off.");
    }
    if(!strcmp(option, "hood", true))
    {
        ...
        return ER_Send(playerid, COLOR_GREEN, bonnet == VEHICLE_PARAMS_ON ? "Vehicle hood opened." : "Vehicle hood closed.");
    }
    if(!strcmp(option, "trunk", true))
    {
        ...
        return ER_Send(playerid, COLOR_GREEN, boot == VEHICLE_PARAMS_ON ? "Vehicle trunk opened." : "Vehicle trunk closed.");
    }
    if(!strcmp(option, "windows", true))
    {
        ...
        return ER_Send(playerid, COLOR_GREEN, VehicleInfo[idx][vWindows][0] ? "Vehicle windows opened." : "Vehicle windows closed.");
    }
