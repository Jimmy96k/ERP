...
stock ER_FactionDialog(playerid, dialogid, response, listitem, inputtext[])
{
    ...
    if(dialogid == DIALOG_FACTION_EDITOR)
    {
        ...
        switch(listitem)
        {
            ...
            default: ER_Send(playerid, COLOR_GREY, "This editor item is saved for the next code pass.");
        }
        #pragma unused fid
        return 1;
    }
    if(dialogid == DIALOG_FACTION_INPUT)
    {
        ...
        else mysql_format(MainPipeline, q, sizeof(q), "UPDATE `factions` SET `motd`='%e' WHERE `id`=%d", inputtext, fid);
