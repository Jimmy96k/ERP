...
stock ER_DoorDialog(playerid, dialogid, response, listitem, const inputtext[])
{
    if(dialogid == DIALOG_DOOR_EDITOR)
    {
        ...
        if(listitem==1){GetPlayerPos(playerid,Doors[idx][dExtX],Doors[idx][dExtY],Doors[idx][dExtZ]);GetPlayerFacingAngle(playerid,Doors[idx][dExtA]);Doors[idx][dExtInt]=GetPlayerInterior(playerid);Doors[idx][dExtVW]=GetPlayerVirtualWorld(playerid);new q[256];mysql_format(MainPipeline,q,sizeof(q),"UPDATE `doors` SET `ext_x`=%f,`ext_y`=%f,`ext_z`=%f,`ext_a`=%f,`ext_int`=%d,`ext_vw`=%d WHERE `id`=%d",Doors[idx][dExtX],Doors[idx][dExtY],Doors[idx][dExtZ],Doors[idx][dExtA],Doors[idx][dExtInt],Doors[idx][dExtVW],did);mysql_tquery(MainPipeline,q);ER_LoadDoors();return ER_Send(playerid,COLOR_GREEN,"Door exterior saved.");}
        ...
        if(listitem==6){Doors[idx][dLockable]=!Doors[idx][dLockable];new q[160];mysql_format(MainPipeline,q,sizeof(q),"UPDATE `doors` SET `lockable`=%d WHERE `id`=%d",Doors[idx][dLockable],did);mysql_tquery(MainPipeline,q);ER_LoadDoors();return ER_Send(playerid,COLOR_GREEN,"Door lockable state updated.");}
        ...
        if(listitem==11){ER_LoadDoors();return ER_Send(playerid,COLOR_GREEN,"Doors reloaded.");}
        if(listitem==12){Doors[idx][dEnabled]=0;new q[128];mysql_format(MainPipeline,q,sizeof(q),"UPDATE `doors` SET `enabled`=0 WHERE `id`=%d",did);mysql_tquery(MainPipeline,q);ER_LoadDoors();return ER_Send(playerid,COLOR_GREEN,"Door deleted/disabled.");}
    ...
    if(dialogid == DIALOG_DOOR_OWNER_TYPE)
    {
        ...
        if(listitem == 0)
        {
            ...
            mysql_tquery(MainPipeline, q); ER_LoadDoors(); return ER_Send(playerid, COLOR_GREEN, "Door owner cleared / set public.");
        }
    ...
    if(dialogid == DIALOG_DOOR_OWNER_FAMILY)
    {
        ...
        mysql_tquery(MainPipeline, q); ER_LoadDoors(); return ER_Send(playerid, COLOR_GREEN, "Door owner set to family.");
    }
    if(dialogid == DIALOG_DOOR_OWNER_FACTION)
    {
        ...
        mysql_tquery(MainPipeline, q); ER_LoadDoors(); return ER_Send(playerid, COLOR_GREEN, "Door owner set to faction.");
    }
    if(dialogid == DIALOG_DOOR_OWNER_VIP)
    {
        ...
        mysql_tquery(MainPipeline, q); ER_LoadDoors(); return ER_Send(playerid, COLOR_GREEN, "Door owner set to VIP level.");
    }
    if(dialogid == DIALOG_DOOR_OWNER_ADMIN)
    {
        ...
        mysql_tquery(MainPipeline, q); ER_LoadDoors(); return ER_Send(playerid, COLOR_GREEN, "Door owner set to admin level.");
    }
    return 0;
}
