...
stock ER_GateDialog(playerid, dialogid, response, listitem, const inputtext[])
{
    ...
    if(dialogid == DIALOG_GATE_EDITOR)
    {
        ...
        if(listitem>=7 && listitem<=11) return ShowPlayerDialog(playerid,DIALOG_GATE_INPUT,DIALOG_STYLE_INPUT,"Gate Setting","Enter value:","Save","Back");
        if(listitem==12) return ER_ToggleGate(playerid,idx);
        if(listitem==13) return ShowPlayerDialog(playerid,DIALOG_GATE_DELETE_CONFIRM,DIALOG_STYLE_MSGBOX,"Delete Gate","Delete/disable this gate?","Delete","Cancel");
        return 1;
    }
