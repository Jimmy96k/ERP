...
{
    new what[24];
    if(sscanf(params, "s[24]", what)) return ER_Send(playerid, COLOR_GREY, "USAGE: /accept [death/family/faction/crew/division/invitation/gun]");
    ...
    return ER_Send(playerid, COLOR_GREY, "USAGE: /accept [death/family/faction/crew/division/invitation/gun]");
}

stock ER_GetHospitalTreatmentTime(playerid, hospitalid, deliveredByEMS)
{
    #pragma unused hospitalid
    if(deliveredByEMS) return 10;
