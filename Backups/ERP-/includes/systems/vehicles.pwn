...
public ER_OnShowVehicleEditor(playerid)
{
	...
	new sqlid, model, c1, c2, paintjob, nos, unlimitednos, hydraulics, wheels, ownerpid, familyid, factionid, jobid, spawnid, ownername[24], owner[96], list[1800];
	new row, line[128];
	...
	format(line, sizeof(line), "Owner Job: %d", jobid);
...
stock ER_VehicleDialog(playerid, dialogid, response, listitem, const inputtext[])
{
	...
	if(dialogid == DIALOG_EDIT_VEHICLE_MENU)
	{
		...
		switch(action)
		{
			...
			case 14: ShowPlayerDialog(playerid, DIALOG_EDIT_VEH_SET_JOB, DIALOG_STYLE_INPUT, "Vehicle Job Owner", "Enter job SQL ID, or 0 to clear:", "Save", "Back");
			case 13:
			{
				new q[128];
				mysql_format(MainPipeline, q, sizeof(q), "UPDATE `vehicles` SET `enabled`=0 WHERE `id`=%d", sqlid);
	...
	if(dialogid == DIALOG_EDIT_VEH_SET_JOB)
	{
		...
		new jobid = strval(inputtext), q[160];
		mysql_format(MainPipeline, q, sizeof(q), "UPDATE `vehicles` SET `job_id`=%d WHERE `id`=%d", jobid, sqlid);
		mysql_tquery(MainPipeline, q, "ER_ReloadVehicles");
		ER_Send(playerid, COLOR_GREEN, "Vehicle job restriction updated.");
		return ER_ShowVehicleEditor(playerid, sqlid);
	}
...
stock ER_OnJobVehicleSeatEntered(playerid, vehicleid)
{
    ...
    if(idx != -1)
    {
        if(VehicleInfo[idx][vJobID] > 0 && !ER_PlayerHasJob(playerid, VehicleInfo[idx][vJobID]))
        {
            ...
            return ER_Send(playerid, COLOR_GREY, "This vehicle is restricted to a specific job.");
        }
    }
    return 1;
}
