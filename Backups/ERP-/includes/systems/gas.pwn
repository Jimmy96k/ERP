...
CMD:editpumps(playerid, params[]){SetPVarInt(playerid,"GasPumpListMode",0); if(!ER_IsAdmin(playerid,ADMIN_SENIOR)) return ER_Send(playerid,COLOR_GREY,"You are not authorized."); new list[2048],line[96]; for(new i;i<GasPumpCount;i++){format(line,sizeof(line),"%d - Gas Pump - Business %d Location %.1f, %.1f, %.1f\n",GasPumps[i][gpSQLID],GasPumps[i][gpBusinessID],GasPumps[i][gpX],GasPumps[i][gpY],GasPumps[i][gpZ]);strcat(list,line,sizeof(list));} if(!list[0]) format(list,sizeof(list),"No gas pumps."); return ShowPlayerDialog(playerid,DIALOG_GAS_PUMP_LIST,DIALOG_STYLE_LIST,"Gas Pumps",list,"Edit","Close");}
