...
stock ER_LoadTurfRowFromCache(row, idx)
{
    cache_get_value_name_int(row, "id", Turfs[idx][tfSQLID]);
    cache_get_value_name(row, "name", Turfs[idx][tfName], 64);
    cache_get_value_name_float(row, "min_x", Turfs[idx][tfMinX]);
    cache_get_value_name_float(row, "min_y", Turfs[idx][tfMinY]);
    cache_get_value_name_float(row, "max_x", Turfs[idx][tfMaxX]);
    cache_get_value_name_float(row, "max_y", Turfs[idx][tfMaxY]);
    cache_get_value_name_int(row, "owner_family", Turfs[idx][tfOwnerFamily]);
    cache_get_value_name_int(row, "owner_faction", Turfs[idx][tfOwnerFaction]);
    cache_get_value_name_int(row, "capture_seconds", Turfs[idx][tfCaptureSeconds]);
    cache_get_value_name_int(row, "enabled", Turfs[idx][tfEnabled]);
    if(Turfs[idx][tfCaptureSeconds] <= 0) Turfs[idx][tfCaptureSeconds] = 90;
    return 1;
}

stock ER_RemoveTurfSlot(idx)
{
    if(idx < 0 || idx >= TurfCount) return 0;
    Turfs[idx][tfEnabled] = 0;
    Turfs[idx][tfSQLID] = 0;
    return 1;
}

stock ER_ReloadTurfBySQLID(sqlid, playerid = INVALID_PLAYER_ID)
{
    new q[128]; mysql_format(MainPipeline, q, sizeof(q), "SELECT * FROM `turfs` WHERE `id`=%d LIMIT 1", sqlid);
    mysql_tquery(MainPipeline, q, "ER_OnSingleTurfReload", "ii", sqlid, playerid);
    return 1;
}

forward ER_OnSingleTurfReload(sqlid, playerid);
public ER_OnSingleTurfReload(sqlid, playerid)
{
    new rows; cache_get_row_count(rows);
    new idx = ER_FindTurfIndexBySQLID(sqlid);
    if(!rows)
    {
        if(idx != -1) ER_RemoveTurfSlot(idx);
        if(playerid != INVALID_PLAYER_ID && IsPlayerConnected(playerid)) ER_Send(playerid, COLOR_GREEN, "Turf reloaded: not found or removed.");
        return 1;
    }
    if(idx == -1)
    {
        if(TurfCount >= MAX_TURFS) return 0;
        idx = TurfCount++;
    }
    ER_LoadTurfRowFromCache(0, idx);
    if(playerid != INVALID_PLAYER_ID && IsPlayerConnected(playerid)) ER_Send(playerid, COLOR_GREEN, "Turf reloaded.");
    return 1;
}
...
{
    for(new i; i < TurfCount; i++) if(Turfs[i][tfEnabled] && ER_PlayerInTurf(playerid, i)) return i;
    return -1;
}
...
{
    new id=cache_insert_id(), msg[96]; ER_ReloadTurfBySQLID(id, playerid); format(msg,sizeof(msg),"Turf created with ID %d.",id); return ER_Send(playerid,COLOR_GREEN,msg);
}

CMD:turfs(playerid, params[])
{
    #pragma unused params
    new body[4096], line[256], owner[96], fam[64], hex[8];
    format(body, sizeof(body), "{F6A800}____________________ Turfs ____________________\n");
    for(new i; i < TurfCount; i++)
    {
        format(hex, sizeof(hex), "FFFFFF");
        if(Turfs[i][tfOwnerFamily] > 0)
        {
            new fidx = ER_FindFamilyIndexBySQLID(Turfs[i][tfOwnerFamily]);
            ER_GetFamilyDisplayNameBySQLID(Turfs[i][tfOwnerFamily], fam, sizeof(fam));
            if(fidx != -1) ER_ColorToDialogHex(Families[fidx][fColor], hex, sizeof(hex));
            format(owner, sizeof(owner), "%s", fam);
            format(line, sizeof(line), "{F6A800}Name: {FFFFFF}%s {F6A800}| Owner: {%s}%s\n", Turfs[i][tfName], hex, owner);
        }
        else
        {
            if(Turfs[i][tfOwnerFaction] > 0) format(owner, sizeof(owner), "Faction %d", Turfs[i][tfOwnerFaction]);
            else format(owner, sizeof(owner), "Unowned");
            format(line, sizeof(line), "{F6A800}Name: {FFFFFF}%s {F6A800}| Owner: {FFFFFF}%s\n", Turfs[i][tfName], owner);
        }
        strcat(body, line, sizeof(body));
    }
    return ShowPlayerDialog(playerid, DIALOG_TURFS_VIEW, DIALOG_STYLE_MSGBOX, "Turfs", body, "Close", "");
}
...
stock ER_TurfDialog(playerid, dialogid, response, listitem, const inputtext[])
{
    ...
    if(dialogid == DIALOG_TURF_INPUT)
    {
        ...
        ER_ReloadTurfBySQLID(id, playerid);
