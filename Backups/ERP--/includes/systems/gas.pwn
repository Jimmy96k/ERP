...
CMD:refuel(playerid, params[])
{
    ...
    new amount;
    if(sscanf(params, "D(25)", amount)) amount = 25;
...
public ER_OnRefuelGasProduct(playerid, bid)
{
    ...
    if(gallonsInStock < amount) return ER_Send(playerid, COLOR_GREY, "This gas station does not have enough gas in stock.");

    new totalPrice = amount * pricePerGallon;
    if(PlayerInfo[playerid][pCash] < totalPrice || GetPlayerMoney(playerid) < totalPrice) return ER_Send(playerid, COLOR_GREY, "You do not have enough cash.");
    ...
    new msg[160];
    format(msg, sizeof(msg), "You refueled %d gallons for %s. Price per gallon: %s.", amount, ER_FormatMoney(totalPrice), ER_FormatMoney(pricePerGallon));
