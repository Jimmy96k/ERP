...
stock ER_DestroyPointWorldSlot(idx)
{
    ...
    Points[idx][ptPickupID] = 0; Points[idx][ptLabelID] = Text3D:0;
    return 1;
}
...
{
    new q[128]; mysql_format(MainPipeline, q, sizeof(q), "SELECT * FROM `points` WHERE `id`=%d LIMIT 1", sqlid);
...
{
    new id = cache_insert_id(), msg[96]; ER_ReloadPointBySQLID(id, playerid); format(msg, sizeof(msg), "Capture point created with ID %d.", id); return ER_Send(playerid, COLOR_GREEN, msg);
}
...
public ER_PointCaptureTick(playerid)
{
    ...
    ER_ReloadPointBySQLID(id); SendClientMessageToAll(COLOR_YELLOW, "A capture point has been taken over.");
    return 1;
}
...
CMD:editpoints(playerid, params[])
{
    ...
    for(new i; i < PointCount; i++)
    {
        format(line, sizeof(line), "%d - %s\n", Points[i][ptSQLID], Points[i][ptName]);
...
stock ER_PointDialog(playerid, dialogid, response, listitem, const inputtext[])
{
    if(dialogid == DIALOG_POINT_LIST)
    {
        ...
        SetPVarInt(playerid, "EditingPoint", Points[listitem][ptSQLID]);
        return ShowPlayerDialog(playerid, DIALOG_POINT_EDITOR, DIALOG_STYLE_LIST, "Point Editor", "Goto\nSet Position Here\nSet Name\nSet Capture Seconds\nDelete", "Select", "Close");
    }
    ...
    if(dialogid == DIALOG_POINT_EDITOR)
    {
        ...
        if(listitem == 3) return ShowPlayerDialog(playerid, DIALOG_POINT_INPUT, DIALOG_STYLE_INPUT, "Capture Seconds", "Enter seconds:", "Save", "Back");
        if(listitem == 4) return ShowPlayerDialog(playerid, DIALOG_POINT_DELETE_CONFIRM, DIALOG_STYLE_MSGBOX, "Delete Point", "Delete this point?", "Delete", "Cancel");
        return 1;
    }
