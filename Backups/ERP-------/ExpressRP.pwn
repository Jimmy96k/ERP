...
public OnGameModeInit()
{
    ...
    ER_LoadTurfs();
    ER_LoadVehicles();
    SetTimer("ER_VehicleFuelTick", 30000, true);
...
public OnPlayerDisconnect(playerid, reason)
{
    ...
    ER_OnGasPlayerDisconnect(playerid);
...
public OnPlayerSpawn(playerid)
{
    ...
    {
        // Do not restore saved HP/weapons while the death system is moving the player
        // through injured/hospital state. Restoring pHealth=0 here caused repeated deaths.
        if(PlayerInfo[playerid][pInjured] || PlayerInfo[playerid][pHospitalized]) return 1;
...
public OnPlayerDeath(playerid, killerid, reason)
{
    ER_PlayerSeatbelt[playerid] = false;
    ER_HideVehicleHUD(playerid);
    if(PlayerInfo[playerid][pHospitalized]) return 1;

    // If a player is already injured/downed and dies again from shots, explosion,
    // vehicle blast, fire, etc., do not restart the floor injury state.
    // Spawn them safely first, then move them directly to hospital treatment.
    if(PlayerInfo[playerid][pInjured])
    {
        ...
        SetTimerEx("ER_SendInjuredDeathToHospital", 400, false, "i", playerid);
        return 1;
    }
...
{
    if(ER_OnMatrunCheckpoint(playerid)) return 1;
    new emscall = GetPVarInt(playerid, "TrackEMSCall");
    if(emscall > 0)
    {
        new target = emscall - 1;
        if(IsPlayerConnected(target))
        {
            ...
            if(IsPlayerInRangeOfPoint(playerid, 8.0, x, y, z))
            {
                ...
                DeletePVar(playerid, "TrackEMSCall");
                ER_Send(playerid, COLOR_GREEN, "You have arrived at the EMS call location.");
                return 1;
            }
        }
        else
...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
    ...
    if(ER_FactionCmdDialog(playerid, dialogid, response, listitem, inputtext)) return 1;
...
public OnPlayerStateChange(playerid, newstate, oldstate)
{
    ...
    {
        // Show the vehicle HUD before any business/job vehicle handler can return early.
        ER_OnVehicleHudSeatEntered(playerid, GetPlayerVehicleID(playerid));
    }
    else if(oldstate == PLAYER_STATE_DRIVER || oldstate == PLAYER_STATE_PASSENGER || newstate == PLAYER_STATE_PASSENGER)
    {
        // HUD is driver-only. Hide it for passengers/on-foot states.
        ER_OnVehicleHudSeatExited(playerid, GetPlayerVehicleID(playerid));
    }
    ...
    if(oldstate == PLAYER_STATE_DRIVER || oldstate == PLAYER_STATE_PASSENGER)
    {
        ...
        if(newstate != PLAYER_STATE_DRIVER) ER_OnVehicleHudSeatExited(playerid, 0);
    }
    return 1;
}
...
public OnPlayerExitVehicle(playerid, vehicleid)
{
    ...
    ER_OnVehicleHudSeatExited(playerid, vehicleid);
    return 1;
}
