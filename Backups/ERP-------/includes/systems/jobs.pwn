...
{
    new label[192];
    format(label, sizeof(label), "{FFFF00}%s\n{FFFFFF}Material Pickup\nPackages: %d | Price: %s\nUse /getmats", Matruns[idx][mrName], Matruns[idx][mrPackageAmount], ER_FormatMoney(Matruns[idx][mrPackageCost]));
    ...
    format(label, sizeof(label), "{FFFF00}%s\n{FFFFFF}Material Dropoff\nReward: %d materials", Matruns[idx][mrName], Matruns[idx][mrMaterialReward]);
...
stock ER_JobDialog(playerid, dialogid, response, listitem, const inputtext[])
{
    ...
    if(dialogid == DIALOG_MRUN_EDIT)
    {
        ...
        switch(listitem)
        {
            case 0: return ShowPlayerDialog(playerid, DIALOG_MRUN_INPUT, DIALOG_STYLE_INPUT, "Matrun Name", "Enter material run name:", "Save", "Back");
            case 1:
            {
            ...
            case 3: return ShowPlayerDialog(playerid, DIALOG_MRUN_INPUT, DIALOG_STYLE_INPUT, "Package Amount", "Enter package amount, e.g. 10:", "Save", "Back");
            case 4: return ShowPlayerDialog(playerid, DIALOG_MRUN_INPUT, DIALOG_STYLE_INPUT, "Package Price", "Enter package price:", "Save", "Back");
            case 5: return ShowPlayerDialog(playerid, DIALOG_MRUN_INPUT, DIALOG_STYLE_INPUT, "Materials Reward", "Enter materials rewarded on delivery:", "Save", "Back");
            case 6: return ShowPlayerDialog(playerid, DIALOG_MRUN_INPUT, DIALOG_STYLE_INPUT, "Pickup Point ID", "Enter pickup point ID, or 0 for none:\nThe pickup point gets the full package price.", "Save", "Back");
            case 7: return ShowPlayerDialog(playerid, DIALOG_MRUN_INPUT, DIALOG_STYLE_INPUT, "Dropoff Point ID", "Enter dropoff/factory point ID, or 0 for none:", "Save", "Back");
            case 8: return ShowPlayerDialog(playerid, DIALOG_MRUN_INPUT, DIALOG_STYLE_INPUT, "Factory Cut Percent", "Enter percentage of the package price added to the dropoff/factory point safe:\nExample: 20", "Save", "Back");
            case 9:
            {
            ...
            case 10: { ER_LoadMatruns(); ER_Send(playerid, COLOR_GREEN, "Material runs reloaded."); return 1; }
            case 11: return ShowPlayerDialog(playerid, DIALOG_MRUN_DELETE, DIALOG_STYLE_MSGBOX, "Delete Material Run", "Disable/delete this material run?", "Delete", "Back");
        }
        return 1;
    }
    if(dialogid == DIALOG_MRUN_INPUT)
    {
        ...
        if(action == 0)
        {
            ...
            mysql_format(MainPipeline, q, sizeof(q), "UPDATE `job_matruns` SET `name`='%e' WHERE `id`=%d", inputtext, id);
        }
        else
        {
            ...
            switch(action)
            {
                case 3: mysql_format(MainPipeline, q, sizeof(q), "UPDATE `job_matruns` SET `package_amount`=%d WHERE `id`=%d", value, id);
                case 4: mysql_format(MainPipeline, q, sizeof(q), "UPDATE `job_matruns` SET `package_cost`=%d WHERE `id`=%d", value, id);
                case 5: mysql_format(MainPipeline, q, sizeof(q), "UPDATE `job_matruns` SET `material_reward`=%d WHERE `id`=%d", value, id);
                case 6: mysql_format(MainPipeline, q, sizeof(q), "UPDATE `job_matruns` SET `pickup_point_id`=%d WHERE `id`=%d", value, id);
                case 7: mysql_format(MainPipeline, q, sizeof(q), "UPDATE `job_matruns` SET `dropoff_point_id`=%d WHERE `id`=%d", value, id);
                case 8:
                {
...
CMD:creatematrun(playerid, params[])
{
    ...
    mysql_format(MainPipeline, q, sizeof(q),
        "INSERT INTO `job_matruns` (`name`,`pickup_x`,`pickup_y`,`pickup_z`,`pickup_int`,`pickup_vw`,`dropoff_x`,`dropoff_y`,`dropoff_z`,`dropoff_int`,`dropoff_vw`,`package_amount`,`package_cost`,`material_reward`,`pickup_point_id`,`dropoff_point_id`,`dropoff_point_cut_percent`,`enabled`) VALUES ('%e',%f,%f,%f,%d,%d,%f,%f,%f,%d,%d,10,75,250,0,0,20,1)",
...
CMD:getmats(playerid, params[])
{
    ...
    new msg[160];
    format(msg, sizeof(msg), "You purchased %d material packages for %s. Deliver them to the destination checkpoint.", Matruns[idx][mrPackageAmount], ER_FormatMoney(cost));
    return ER_Send(playerid, COLOR_GREEN, msg);
}
