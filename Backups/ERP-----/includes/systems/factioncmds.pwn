...
forward ER_FinishLoadPatient(medicid, patientid, vehicleid, seatid);
...
stock ER_IsPlayerNearFacLocker(playerid, Float:range = 6.0)
{
    if(PlayerInfo[playerid][pFaction] <= 0) return 0;
    new q[256];
    mysql_format(MainPipeline, q, sizeof(q), "SELECT `x`,`y`,`z`,`interior`,`vw` FROM `faction_lockers` WHERE `faction_id`=%d AND `enabled`=1 LIMIT 1", PlayerInfo[playerid][pFaction]);
    ...
    cache_delete(res);
    return GetPlayerInterior(playerid) == interior && GetPlayerVirtualWorld(playerid) == vw && IsPlayerInRangeOfPoint(playerid, range, x, y, z);
}

stock ER_IsFactionVehicleForPlayer(playerid, vehicleid)
{
    if(vehicleid == INVALID_VEHICLE_ID || vehicleid == 0) return 0;
    if(PlayerInfo[playerid][pFaction] <= 0) return 0;
    new idx = ER_FindVehicleBySpawnID(vehicleid);
    if(idx == -1) return 0;
    return (VehicleInfo[idx][vFactionID] == PlayerInfo[playerid][pFaction]);
}

stock ER_FindFreePassengerSeat(vehicleid)
{
...
// Type: Law Enforcement Faction CMDS
...
CMD:arrest(playerid, params[])
{
    ...
    if(!ER_FactionCommandAllowed(playerid, 1)) return ER_Send(playerid, COLOR_GREY, "You are not in a law enforcement faction.");
    if(!IsPlayerConnected(target) || !PlayerInfo[target][pLoggedIn]) return ER_Send(playerid, COLOR_GREY, "Invalid player.");
    if(!GetPVarInt(target, "Cuffed")) return ER_Send(playerid, COLOR_GREY, "The suspect must be cuffed before arrest.");
    if(!ER_IsPlayerNearPlayer(playerid, target, 5.0)) return ER_Send(playerid, COLOR_GREY, "You are not close enough.");
    if(!ER_IsPlayerNearFacLocker(playerid, 8.0)) return ER_Send(playerid, COLOR_GREY, "You must be at your faction processing/locker point to arrest.");
...
{
    return cmd_undrag(playerid, params);
}
...
CMD:deliverpatient(playerid, params[])
{
    ...
    if(!IsPlayerInAnyVehicle(playerid)) return ER_Send(playerid, COLOR_GREY, "You must be inside the ambulance/vehicle at the EMS delivery point.");
    if(!IsPlayerInAnyVehicle(target)) return ER_Send(playerid, COLOR_GREY, "The patient must be inside your vehicle.");

    new veh = GetPlayerVehicleID(playerid);
    if(GetPlayerVehicleID(target) != veh) return ER_Send(playerid, COLOR_GREY, "The patient must be inside the same vehicle as you.");
    if(!ER_IsFactionVehicleForPlayer(playerid, veh)) return ER_Send(playerid, COLOR_GREY, "You must deliver patients using an EMS vehicle owned by your faction.");

    new hospitalid = ER_FindEMSDeliveryHosp(playerid, 10.0);
    if(hospitalid == -1) return ER_Send(playerid, COLOR_GREY, "You must be at a hospital EMS delivery point.");

    PlayerInfo[target][pDeliveredByEMS] = 1;
    ER_SendToSpecificHospital(target, hospitalid, 1);

    new hslot = ER_FindHospitalSlotByID(hospitalid), msg[160];
    if(hslot != -1) format(msg, sizeof(msg), "Patient delivered to %s. No hospital timer was applied.", Hospitals[hslot][hName]);
    else format(msg, sizeof(msg), "Patient delivered to hospital. No hospital timer was applied.");
    return ER_Send(playerid, COLOR_GREEN, msg);
}


// Type: News Faction CMDS
...
// Type: Emergency Shared Faction CMDS
...
CMD:tazer(playerid, params[])
{
    ...
    if(GetPVarInt(playerid, "TazerEquipped")) { GivePlayerWeapon(playerid, 23, 99999); return ER_Send(playerid, COLOR_GREEN, "Tazer equipped."); }
...
{
    new type[24];
    if(sscanf(params, "s[24]", type)) return ER_Send(playerid, COLOR_GREY, "USAGE: /service [ems]");
    if(strcmp(type, "ems", true)) return ER_Send(playerid, COLOR_GREY, "USAGE: /service [ems]");
    if(!PlayerInfo[playerid][pInjured]) return ER_Send(playerid, COLOR_GREY, "You can only request EMS while injured.");

    new Float:x, Float:y, Float:z, zone[32], pname[MAX_PLAYER_NAME], msg[180];
    GetPlayerPos(playerid, x, y, z);
    ER_GetPlayerZone(playerid, zone, sizeof(zone));
    ER_GetDisplayName(playerid, pname, sizeof(pname));

    SetPVarInt(playerid, "EMSRequestActive", 1);
    SetPVarFloat(playerid, "EMSRequestX", x);
    SetPVarFloat(playerid, "EMSRequestY", y);
    SetPVarFloat(playerid, "EMSRequestZ", z);
    SetPVarInt(playerid, "EMSRequestInt", GetPlayerInterior(playerid));
    SetPVarInt(playerid, "EMSRequestVW", GetPlayerVirtualWorld(playerid));

    format(msg, sizeof(msg), "EMS Dispatch: %s requested EMS at %s. Use /accept ems to respond.", pname, zone);
    foreach(new i : Player) if(PlayerInfo[i][pLoggedIn] && ER_FactionCommandAllowed(i, 2)) SendClientMessage(i, COLOR_LIGHTBLUE, msg);

    return ER_Send(playerid, COLOR_LIGHTBLUE, "You have requested EMS. Please wait for an EMS unit to respond.");
}

stock ER_CountActiveEMSCalls()
{
    new count;
    foreach(new i : Player)
    {
        if(!PlayerInfo[i][pLoggedIn]) continue;
        if(!GetPVarInt(i, "EMSRequestActive")) continue;
        if(!PlayerInfo[i][pInjured]) { DeletePVar(i, "EMSRequestActive"); continue; }
        count++;
    }
    return count;
}

stock ER_AcceptSpecificEMSCall(playerid, caller)
{
    if(!ER_FactionCommandAllowed(playerid, 2)) return ER_Send(playerid, COLOR_GREY, "You are not in EMS/Fire faction.");
    if(!IsPlayerConnected(caller) || !PlayerInfo[caller][pLoggedIn] || !GetPVarInt(caller, "EMSRequestActive") || !PlayerInfo[caller][pInjured])
        return ER_Send(playerid, COLOR_GREY, "That EMS call is no longer active.");

    new Float:x = GetPVarFloat(caller, "EMSRequestX");
    new Float:y = GetPVarFloat(caller, "EMSRequestY");
    new Float:z = GetPVarFloat(caller, "EMSRequestZ");
    new medic[MAX_PLAYER_NAME], pname[MAX_PLAYER_NAME], msg[180];

    ER_GetDisplayName(playerid, medic, sizeof(medic));
    ER_GetDisplayName(caller, pname, sizeof(pname));

    SetPlayerCheckpoint(playerid, x, y, z, 4.0);
    SetPVarInt(playerid, "TrackEMSCall", caller + 1);
    DeletePVar(caller, "EMSRequestActive");

    format(msg, sizeof(msg), "EMS Dispatch: %s accepted %s's EMS call.", medic, pname);
    foreach(new i : Player) if(PlayerInfo[i][pLoggedIn] && ER_FactionCommandAllowed(i, 2)) SendClientMessage(i, COLOR_LIGHTBLUE, msg);

    format(msg, sizeof(msg), "EMS unit %s has accepted your call and is responding.", medic);
    ER_Send(caller, COLOR_LIGHTBLUE, msg);
    return ER_Send(playerid, COLOR_GREEN, "EMS call accepted. The injured player's location has been marked.");
}

stock ER_ShowEMSCallList(playerid)
{
    if(!ER_FactionCommandAllowed(playerid, 2)) return ER_Send(playerid, COLOR_GREY, "You are not in EMS/Fire faction.");

    new list[768], line[96], name[MAX_PLAYER_NAME], zone[32], count;
    foreach(new i : Player)
    {
        if(!PlayerInfo[i][pLoggedIn]) continue;
        if(!GetPVarInt(i, "EMSRequestActive")) continue;
        ...
        ER_GetDisplayName(i, name, sizeof(name));
        ER_GetPlayerZone(i, zone, sizeof(zone));
        format(line, sizeof(line), "%d - %s - %s\n", count + 1, name, zone);
        strcat(list, line, sizeof(list));
        new key[24]; format(key, sizeof(key), "EMSCall_%d", count);
        SetPVarInt(playerid, key, i + 1);
        count++;
    }

    SetPVarInt(playerid, "EMSCallCount", count);
    if(!count) return ER_Send(playerid, COLOR_GREY, "There are no active EMS calls.");
    return ShowPlayerDialog(playerid, DIALOG_EMS_CALLS, DIALOG_STYLE_LIST, "Active EMS Calls", list, "Accept", "Close");
}

stock ER_AcceptEMSCall(playerid)
{
    if(!ER_FactionCommandAllowed(playerid, 2)) return ER_Send(playerid, COLOR_GREY, "You are not in EMS/Fire faction.");

    new count = ER_CountActiveEMSCalls();
    if(count <= 0) return ER_Send(playerid, COLOR_GREY, "There are no active EMS calls.");
    return ER_ShowEMSCallList(playerid);
}

stock ER_FactionCmdDialog(playerid, dialogid, response, listitem, inputtext[])
{
    #pragma unused inputtext
    if(dialogid != DIALOG_EMS_CALLS) return 0;
    if(!response) return 1;
    if(listitem < 0 || listitem >= GetPVarInt(playerid, "EMSCallCount")) return 1;
    new key[24]; format(key, sizeof(key), "EMSCall_%d", listitem);
    new caller = GetPVarInt(playerid, key) - 1;
    return ER_AcceptSpecificEMSCall(playerid, caller);
}


stock ER_FindNearFacVeh(playerid, Float:range = 7.0)
{
    if(PlayerInfo[playerid][pFaction] <= 0) return INVALID_VEHICLE_ID;

    for(new i = 0; i < VehicleCount; i++)
    {
        if(VehicleInfo[i][vFactionID] != PlayerInfo[playerid][pFaction]) continue;
        if(VehicleInfo[i][vSpawnedID] == INVALID_VEHICLE_ID || VehicleInfo[i][vSpawnedID] == 0) continue;
        if(GetVehicleVirtualWorld(VehicleInfo[i][vSpawnedID]) != GetPlayerVirtualWorld(playerid)) continue;

        new Float:vx, Float:vy, Float:vz;
        GetVehiclePos(VehicleInfo[i][vSpawnedID], vx, vy, vz);

        if(GetPlayerDistanceFromPoint(playerid, vx, vy, vz) <= range)
            return VehicleInfo[i][vSpawnedID];
    }
    return INVALID_VEHICLE_ID;
}

stock cmd_loadpt(playerid, params[])
{
    new target;
    if(sscanf(params, "u", target)) return ER_Send(playerid, COLOR_GREY, "USAGE: /loadpt [player]");
    if(!ER_FactionCommandAllowed(playerid, 2)) return ER_Send(playerid, COLOR_GREY, "You are not in EMS/Fire faction.");
    if(!IsPlayerConnected(target) || !PlayerInfo[target][pLoggedIn]) return ER_Send(playerid, COLOR_GREY, "Invalid player.");
    if(!PlayerInfo[target][pInjured]) return ER_Send(playerid, COLOR_GREY, "That player is not injured.");
    if(IsPlayerInAnyVehicle(playerid)) return ER_Send(playerid, COLOR_GREY, "Exit the vehicle first, then use /loadpt near the patient and EMS vehicle.");
    if(IsPlayerInAnyVehicle(target)) return ER_Send(playerid, COLOR_GREY, "That patient is already inside a vehicle.");
    if(!ER_IsPlayerNearPlayer(playerid, target, 6.0)) return ER_Send(playerid, COLOR_GREY, "You are not close enough to the patient.");

    new veh = ER_FindNearFacVeh(playerid, 8.0);
    if(veh == INVALID_VEHICLE_ID) return ER_Send(playerid, COLOR_GREY, "You must be near an EMS vehicle owned by your faction.");

    new Float:vx, Float:vy, Float:vz;
    GetVehiclePos(veh, vx, vy, vz);
    if(!IsPlayerInRangeOfPoint(target, 10.0, vx, vy, vz))
        return ER_Send(playerid, COLOR_GREY, "The patient must also be near the EMS vehicle. Drag them closer first.");

    new seat = ER_FindFreePassengerSeat(veh);
    if(seat == -1) return ER_Send(playerid, COLOR_GREY, "No free passenger seat was found in that vehicle.");

    DeletePVar(target, "DraggedBy");
    ClearAnimations(target);
    TogglePlayerControllable(target, 1);
    SetPlayerVirtualWorld(target, GetVehicleVirtualWorld(veh));
    SetPlayerPos(target, vx + 1.5, vy, vz + 0.5);
    PutPlayerInVehicle(target, veh, seat);
    SetTimerEx("ER_FinishLoadPatient", 350, false, "iiii", playerid, target, veh, seat);

    new pname[MAX_PLAYER_NAME], msg[144];
    ER_GetDisplayName(target, pname, sizeof(pname));
    format(msg, sizeof(msg), "You loaded %s into your faction EMS vehicle.", pname);
    ER_Send(playerid, COLOR_GREEN, msg);
    return ER_Send(target, COLOR_LIGHTBLUE, "An EMS medic loaded you into the ambulance.");
}

public ER_FinishLoadPatient(medicid, patientid, vehicleid, seatid)
{
    if(!IsPlayerConnected(patientid)) return 0;
    if(!PlayerInfo[patientid][pInjured]) return 0;

    if(!IsPlayerInVehicle(patientid, vehicleid))
    {
        PutPlayerInVehicle(patientid, vehicleid, seatid);
    }

    SetPlayerHealth(patientid, 35.0);
    ClearAnimations(patientid);
    TogglePlayerControllable(patientid, 0);

    if(IsPlayerConnected(medicid) && !IsPlayerInVehicle(patientid, vehicleid))
        ER_Send(medicid, COLOR_GREY, "Patient load failed. Move closer to the vehicle and try again.");
...
CMD:loadpt(playerid, params[]) { return cmd_loadpt(playerid, params); }
...
{
    if(!ER_IsAdmin(playerid, ADMIN_SENIOR) && (!ER_IsEMSFireFaction(playerid) || PlayerInfo[playerid][pFactionRank] < 5)) return ER_Send(playerid, COLOR_GREY, "You need EMS rank 5+ or Senior Admin.");
    new Float:x,Float:y,Float:z; GetPlayerPos(playerid,x,y,z);
...
// Type: Shared Family/Faction Utility CMDS
CMD:locker(playerid, params[])
{
    if(PlayerInfo[playerid][pFaction] > 0)
    {
        if(!ER_IsPlayerAtFactionLocker(playerid, PlayerInfo[playerid][pFaction])) return ER_Send(playerid, COLOR_GREY, "You are not near your faction locker.");
    ...
    {
        if(!ER_IsPlayerAtFamilyLocker(playerid, PlayerInfo[playerid][pFamily])) return ER_Send(playerid, COLOR_GREY, "You are not near your family locker.");
...
stock cmd_drag(playerid, params[])
{
    ...
    if(!IsPlayerConnected(target) || !PlayerInfo[target][pLoggedIn])
        return ER_Send(playerid, COLOR_GREY, "Invalid player.");

    if(target == playerid)
        return ER_Send(playerid, COLOR_GREY, "You cannot drag yourself.");

    if(!ER_IsPlayerNearPlayer(playerid, target, 5.0))
        return ER_Send(playerid, COLOR_GREY, "You are not close enough.");

    new isLaw = ER_FactionCommandAllowed(playerid, 1);
    new isEMS = ER_FactionCommandAllowed(playerid, 2);

    if(!isLaw && !isEMS)
        return ER_Send(playerid, COLOR_GREY, "You are not authorized.");

    if(isLaw && GetPVarInt(target, "Cuffed"))
    {
        SetPVarInt(target, "DraggedBy", playerid + 1);
        ER_Send(target, COLOR_LIGHTBLUE, "You are now being dragged by law enforcement.");
        return ER_Send(playerid, COLOR_GREEN, "You started dragging the cuffed player. Use /undrag [player] to stop.");
    }

    if(isEMS && PlayerInfo[target][pInjured])
    {
        SetPVarInt(target, "DraggedBy", playerid + 1);
        ER_Send(target, COLOR_LIGHTBLUE, "You are now being dragged by EMS.");
        return ER_Send(playerid, COLOR_GREEN, "You started dragging the injured patient. Use /undrag [player] to stop.");
    }

    if(isLaw)
        return ER_Send(playerid, COLOR_GREY, "Police can only drag cuffed players.");

    return ER_Send(playerid, COLOR_GREY, "EMS can only drag injured patients.");
}

stock cmd_undrag(playerid, params[])
{
    new target;
    if(sscanf(params, "u", target)) return ER_Send(playerid, COLOR_GREY, "USAGE: /undrag [player]");

    if(!IsPlayerConnected(target) || !PlayerInfo[target][pLoggedIn])
        return ER_Send(playerid, COLOR_GREY, "Invalid player.");

    if(GetPVarInt(target, "DraggedBy") != playerid + 1)
        return ER_Send(playerid, COLOR_GREY, "You are not dragging that player.");

    DeletePVar(target, "DraggedBy");
    TogglePlayerControllable(target, 1);

    ER_Send(target, COLOR_LIGHTBLUE, "You are no longer being dragged.");
    return ER_Send(playerid, COLOR_GREEN, "You stopped dragging the player.");
}
