...
new bool:TempVehicle[MAX_VEHICLES];
...
public ER_OnVehiclesLoad()
{
    ...
    for(new r; r < rows && VehicleCount < MAX_DYNAMIC_VEHICLES; r++)
    {
        ...
        cache_get_value_name_float(r, "fuel_consumption_rate", VehicleInfo[VehicleCount][vFuelConsumptionRate]);
        if(VehicleInfo[VehicleCount][vFuelConsumptionRate] <= 0.0) VehicleInfo[VehicleCount][vFuelConsumptionRate] = ServerCore[scDefaultVehicleFuelConsumption];
        if(VehicleInfo[VehicleCount][vFuelConsumptionRate] <= 0.0) VehicleInfo[VehicleCount][vFuelConsumptionRate] = 1.0;
...
CMD:aveh(playerid, params[])
{
    ...
    if(sscanf(params, "s[32]D(0)D(0)", modelstr, c1, c2))
        return ER_Send(playerid, COLOR_GREY, "USAGE: /aveh [model/name] [color1 optional] [color2 optional]");

    model = ER_FindVehicleModel(modelstr);

    if(model < 400 || model > 611)
        return ER_Send(playerid, COLOR_GREY, "Invalid vehicle model/name.");
    ...
    // Temporary admin vehicles are not saved/editable, but they still get HUD support.
    TempVehicle[veh] = true;

    PutPlayerInVehicle(playerid, veh, 0);

    SetTimerEx("ER_ShowVehicleHUDAfterEnter", 250, false, "i", playerid);
...
CMD:createveh(playerid, params[])
{
    ...
    x += (3.0 * floatsin(-a, degrees));
    y += (3.0 * floatcos(-a, degrees));
    mysql_format(MainPipeline, q, sizeof(q), "INSERT INTO `vehicles` (`owner_pid`,`family_id`,`faction_id`,`job_id`,`model`,`color1`,`color2`,`paintjob`,`x`,`y`,`z`,`a`,`interior`,`vw`,`lock_type`,`fuel`,`fuel_consumption_rate`,`enabled`) VALUES (0,0,0,0,%d,%d,%d,-1,%f,%f,%f,%f,%d,%d,0,100.0,%f,1)",
        model, c1, c2, x, y, z, a, GetPlayerInterior(playerid), GetPlayerVirtualWorld(playerid), ServerCore[scDefaultVehicleFuelConsumption]);
...
public ER_OnAdminVehicleCreated(playerid)
{
    ...
    if(VehicleCount < MAX_DYNAMIC_VEHICLES)
    {
        ...
        VehicleInfo[idx][vFuelConsumptionRate] = ServerCore[scDefaultVehicleFuelConsumption];
...
CMD:createpveh(playerid, params[])
{
    ...
    x += (3.0 * floatsin(-a, degrees));
    y += (3.0 * floatcos(-a, degrees));
    mysql_format(MainPipeline, q, sizeof(q), "INSERT INTO `vehicles` (`owner_pid`,`family_id`,`faction_id`,`job_id`,`model`,`color1`,`color2`,`paintjob`,`x`,`y`,`z`,`a`,`interior`,`vw`,`fuel`,`fuel_consumption_rate`,`enabled`) VALUES (%d,0,0,0,%d,%d,%d,-1,%f,%f,%f,%f,%d,%d,100.0,%f,1)",
        pid, model, c1, c2, x, y, z, a, GetPlayerInterior(playerid), GetPlayerVirtualWorld(playerid), ServerCore[scDefaultVehicleFuelConsumption]);
...
public ER_OnAllVehiclesDialog(playerid)
{
	...
	for(new r; r < rows; r++)
	{
		...
		if(jobid > 0)
		{
			...
			format(owner, sizeof(owner), "Job Type (%d) %s", jobid, jobName);
		}
...
public ER_OnShowVehicleEditor(playerid)
{
	...
	if(idx != -1) format(line, sizeof(line), "Fuel Consumption: %.2f gal/km", VehicleInfo[idx][vFuelConsumptionRate]);
	else format(line, sizeof(line), "Fuel Consumption: %.2f gal/km", ServerCore[scDefaultVehicleFuelConsumption]);
	ER_AddVehicleEditorRow(playerid, row, list, sizeof(list), 16, line);
...
stock ER_VehicleDialog(playerid, dialogid, response, listitem, const inputtext[])
{
	...
	if(dialogid == DIALOG_EDIT_VEHICLE_MENU)
	{
		...
		switch(action)
		{
			...
			case 16:
			{
				new idx = ER_FindVehicleBySQLID(sqlid), msg[192];
				if(idx == -1) return ER_ShowVehicleEditor(playerid, sqlid);
				format(msg, sizeof(msg), "Enter fuel consumption rate in gallons per KM:\n\nCurrent: %.2f gal/km\nExample: 1.0 = 1 gallon per 1 KM", VehicleInfo[idx][vFuelConsumptionRate]);
				return ShowPlayerDialog(playerid, DIALOG_EDIT_VEH_CONSUMPTION, DIALOG_STYLE_INPUT, "Vehicle Fuel Consumption", msg, "Save", "Back");
			}
		}
		return 1;
	}
	...
	if(dialogid == DIALOG_EDIT_VEH_CONSUMPTION)
	{
		new sqlid = GetPVarInt(playerid, "EditingVehicleID");
		if(!response) return ER_ShowVehicleEditor(playerid, sqlid);
		new idx = ER_FindVehicleBySQLID(sqlid);
		if(idx == -1) return ER_ShowVehicleEditor(playerid, sqlid);
		new Float:rate = floatstr(inputtext);
		if(rate <= 0.0 || rate > 20.0) return ER_Send(playerid, COLOR_GREY, "Fuel consumption must be higher than 0.0 and not above 20.0 gal/km.");
		VehicleInfo[idx][vFuelConsumptionRate] = rate;
		new q[192];
		mysql_format(MainPipeline, q, sizeof(q), "UPDATE `vehicles` SET `fuel_consumption_rate`=%f WHERE `id`=%d", rate, sqlid);
		mysql_tquery(MainPipeline, q);
		ER_Send(playerid, COLOR_GREEN, "Vehicle fuel consumption updated.");
		return ER_ShowVehicleEditor(playerid, sqlid);
	}
	...
	if(dialogid == DIALOG_EDIT_VEH_OWNER)
	{
		...
		if(listitem == 3)
		{
			return ShowPlayerDialog(playerid, DIALOG_EDIT_VEH_SET_JOB, DIALOG_STYLE_INPUT, "Vehicle Job Type Owner", "Enter job type ID, or 0 to clear:\n1 Arms Dealer\n2 Craftsman\n3 Trucker\n4 Mechanic\n5 Taxi\n6 Garbage Man\n7 Lawyer\n8 Detective\n9 Drug Dealer\n10 Drug Smuggler\n11 Pizza Boy", "Save", "Back");
		}
...
stock ER_IsVehicleBikeLike(model)
{
    switch(model)
    {
        // Bikes, BMX/mountain bikes, quads, kart and light buggy-style vehicles.
...
stock ER_UpdateVehicleHUD(playerid)
{
    ...
    new vehicleid = GetPlayerVehicleID(playerid);
    new idx = ER_FindVehicleBySpawnID(vehicleid);
    new bool:isTemp = TempVehicle[vehicleid] ? true : false;
    ...
    if(isTemp || (idx != -1 && VehicleInfo[idx][vUnlimitedFuel]))
    {
        format(line, sizeof(line), "~b~Fuel: ~r~U");
    }
    else
    {
    ...
    if(idx != -1) format(line, sizeof(line), "~b~Mileage: ~w~%.1f KM", VehicleInfo[idx][vMileage]);
    else format(line, sizeof(line), "~b~Mileage: ~w~0.0 KM");
    PlayerTextDrawSetString(playerid, ER_VehHudMileage[playerid], line);

    new bool:bikeLike = ER_IsVehicleBikeLike(model) ? true : false;
    if(bikeLike) format(line, sizeof(line), "~b~Helmet: %s", ER_PlayerSeatbelt[playerid] ? "~g~On" : "~r~Off");
    else format(line, sizeof(line), "~b~Seatbelt: %s", ER_PlayerSeatbelt[playerid] ? "~g~Fastened" : "~r~Not Fastened");
...
stock ER_ShowVehicleHUD(playerid)
{
    ...
    if(ER_IsVehicleBikeLike(GetVehicleModel(GetPlayerVehicleID(playerid))))
    {
        PlayerTextDrawHide(playerid, ER_VehHudHood[playerid]);
        PlayerTextDrawHide(playerid, ER_VehHudTrunk[playerid]);
    }
        ...
        if(ER_VehHudLastPosValid[i])
        {
            ...
            if(dist > 0.50 && dist < 150.0)
            {
                ...
                if(engine == VEHICLE_PARAMS_ON)
                {
                    if(idx != -1)
                    {
                        new Float:km = dist / 1000.0;
                        VehicleInfo[idx][vMileage] += km;
                        if(!VehicleInfo[idx][vUnlimitedFuel] && VehicleInfo[idx][vFuel] > 0.0)
                        {
                            new Float:rate = VehicleInfo[idx][vFuelConsumptionRate];
                            if(rate <= 0.0) rate = ServerCore[scDefaultVehicleFuelConsumption];
                            if(rate <= 0.0) rate = 1.0;
                            ER_SetVehicleFuel(idx, VehicleInfo[idx][vFuel] - (km * rate));
                        }
                    }
                }
            }
...
public ER_VehicleFuelTick()
{
    foreach(new i : Player)
    {
        ...
        if(engine == VEHICLE_PARAMS_ON)
        {
            if(VehicleInfo[idx][vUnlimitedFuel])
            {
                // Unlimited fuel vehicles do not consume fuel.
            }
            else if(VehicleInfo[idx][vFuel] <= 0.0)
            {
            ...
            else if(ServerCore[scVehicleIdleFuelEnabled])
            {
                new Float:vx, Float:vy, Float:vz;
                GetVehicleVelocity(vehicleid, vx, vy, vz);
                ...
                if(speed < 0.015)
                {
                    new Float:idle = ServerCore[scVehicleIdleFuelGallonsPerTick];
                    if(idle < 0.0) idle = 0.0;
                    if(idle > 1.0) idle = 1.0;
                    if(idle > 0.0) ER_SetVehicleFuel(idx, VehicleInfo[idx][vFuel] - idle);
                }
            }
...
stock ER_HandleVehicleCommand(playerid, params[])
{
    ...
    {
        new bool:hasKeys = (TempVehicle[vehicleid] || ER_PlayerCanUseVehicle(playerid, vehicleid)) ? true : false;
        if(engine == VEHICLE_PARAMS_ON)
        {
            engine = VEHICLE_PARAMS_OFF;
            SetVehicleParamsEx(vehicleid, engine, lights, alarm, doors, bonnet, boot, objective);
        ...
        new idx = ER_FindVehicleBySpawnID(vehicleid);
        if(!TempVehicle[vehicleid] && idx != -1 && !VehicleInfo[idx][vUnlimitedFuel] && VehicleInfo[idx][vFuel] <= 0.0)
        {
...
public ER_OnIgnitionTimer(playerid)
{
    ...
    {
        new idx = ER_FindVehicleBySpawnID(vehicleid);
        if(!TempVehicle[vehicleid] && idx != -1 && !VehicleInfo[idx][vUnlimitedFuel] && VehicleInfo[idx][vFuel] <= 0.0) return ER_Send(playerid, COLOR_GREY, "This vehicle is out of fuel. Refill it before starting the engine.");
    }
...
{
    #pragma unused params
    if(GetPlayerState(playerid) != PLAYER_STATE_DRIVER) return ER_Send(playerid, COLOR_GREY, "You must be driving a vehicle to use this.");
    ...
    if(bikeLike)
    {
        ER_JobRP(playerid, ER_PlayerSeatbelt[playerid] ? "puts on a helmet." : "removes their helmet.");
        ER_Send(playerid, COLOR_GREY, ER_PlayerSeatbelt[playerid] ? "Helmet equipped." : "Helmet removed.");
    }
    else
    {
        ER_JobRP(playerid, ER_PlayerSeatbelt[playerid] ? "fastens their seatbelt." : "unfastens their seatbelt.");
...
{
    if(GetPlayerState(playerid) != PLAYER_STATE_DRIVER) return ER_Send(playerid, COLOR_GREY, "You must be driving a bike or buggy to use a helmet.");
    if(!ER_IsVehicleBikeLike(GetVehicleModel(GetPlayerVehicleID(playerid)))) return ER_Send(playerid, COLOR_GREY, "You can only use a helmet on bikes or buggy-style vehicles.");
