...
CMD:editturfs(playerid, params[]) { if(!ER_IsAdmin(playerid, ADMIN_HEAD)) return ER_Send(playerid,COLOR_GREY,"You are not authorized."); new list[2048],line[96]; for(new i;i<TurfCount;i++){format(line,sizeof(line),"%d - %s\n",Turfs[i][tfSQLID],Turfs[i][tfName]);strcat(list,line,sizeof(list));} if(!list[0]) format(list,sizeof(list),"No turfs."); return ShowPlayerDialog(playerid,DIALOG_TURF_LIST,DIALOG_STYLE_LIST,"Turfs",list,"Edit","Close"); }
CMD:editturf(playerid, params[]) { new id; if(!ER_IsAdmin(playerid, ADMIN_HEAD)) return ER_Send(playerid,COLOR_GREY,"You are not authorized."); if(sscanf(params,"d",id)) return ER_Send(playerid,COLOR_GREY,"USAGE: /editturf [id]"); SetPVarInt(playerid,"EditingTurf",id); return ShowPlayerDialog(playerid,DIALOG_TURF_EDITOR,DIALOG_STYLE_LIST,"Turf Editor","Set Center/Radius Here\nSet Name\nSet Capture Seconds\nDelete", "Select", "Close"); }
...
{
    if(dialogid==DIALOG_TURF_LIST){if(!response)return 1;if(listitem<0||listitem>=TurfCount)return 1;SetPVarInt(playerid,"EditingTurf",Turfs[listitem][tfSQLID]);return ShowPlayerDialog(playerid,DIALOG_TURF_EDITOR,DIALOG_STYLE_LIST,"Turf Editor","Set Center/Radius Here\nSet Name\nSet Capture Seconds\nDelete", "Select", "Close");}
    if(dialogid==DIALOG_TURF_EDITOR){if(!response)return 1;SetPVarInt(playerid,"TurfEditAction",listitem);if(listitem==0)return ShowPlayerDialog(playerid,DIALOG_TURF_INPUT,DIALOG_STYLE_INPUT,"Turf Radius","Enter radius around your current position:","Save","Back");if(listitem==1)return ShowPlayerDialog(playerid,DIALOG_TURF_INPUT,DIALOG_STYLE_INPUT,"Turf Name","Enter name:","Save","Back");if(listitem==2)return ShowPlayerDialog(playerid,DIALOG_TURF_INPUT,DIALOG_STYLE_INPUT,"Capture Seconds","Enter seconds:","Save","Back");if(listitem==3)return ShowPlayerDialog(playerid,DIALOG_TURF_DELETE_CONFIRM,DIALOG_STYLE_MSGBOX,"Delete Turf","Delete this turf?","Delete","Cancel");}
    if(dialogid==DIALOG_TURF_INPUT){if(!response)return 1;new id=GetPVarInt(playerid,"EditingTurf"),action=GetPVarInt(playerid,"TurfEditAction"),q[256]; if(action==0){new radius=strval(inputtext);if(radius<10)radius=10;if(radius>500)radius=500;new Float:x,Float:y,Float:z;GetPlayerPos(playerid,x,y,z);mysql_format(MainPipeline,q,sizeof(q),"UPDATE `turfs` SET `min_x`=%f,`min_y`=%f,`max_x`=%f,`max_y`=%f WHERE `id`=%d",x-radius,y-radius,x+radius,y+radius,id);} else if(action==1) mysql_format(MainPipeline,q,sizeof(q),"UPDATE `turfs` SET `name`='%e' WHERE `id`=%d",inputtext,id); else mysql_format(MainPipeline,q,sizeof(q),"UPDATE `turfs` SET `capture_seconds`=%d WHERE `id`=%d",strval(inputtext),id); mysql_tquery(MainPipeline,q);ER_LoadTurfs();return ER_Send(playerid,COLOR_GREEN,"Turf updated.");}
    if(dialogid==DIALOG_TURF_DELETE_CONFIRM){if(!response)return 1;new id=GetPVarInt(playerid,"EditingTurf"),q[128];mysql_format(MainPipeline,q,sizeof(q),"UPDATE `turfs` SET `enabled`=0 WHERE `id`=%d",id);mysql_tquery(MainPipeline,q);ER_LoadTurfs();return ER_Send(playerid,COLOR_GREEN,"Turf deleted.");}
    return 0;
}
