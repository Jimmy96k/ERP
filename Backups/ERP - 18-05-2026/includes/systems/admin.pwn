...
stock ER_ShowAdminGotoList(playerid, type)
{
    ...
    if(type == 4)
    {
        ...
        {
            format(line, sizeof(line), "%d - %s\n", VehicleInfo[i][vSQLID], ER_GetVehicleModelName(VehicleInfo[i][vModel]));
            strcat(list, line, sizeof(list));
        ...
        SetPVarInt(playerid, "AdminGotoCount", count);
        return ShowPlayerDialog(playerid, DIALOG_ADMIN_GOTO_LIST, DIALOG_STYLE_LIST, "Goto Vehicle", list, "Goto", "Close");
    }
    return 1;
}
...
stock ER_GiveGunByListIndex(playerid, target, listitem)
{
    ...
    GivePlayerWeapon(target, wid, 500);
...
CMD:givegun(playerid, params[])
{
    ...
    if(sscanf(params, "uS()[32]D(500)", target, weapon, ammo)) return ER_ShowGiveGunList(playerid, playerid);
    if(target == INVALID_PLAYER_ID || !IsPlayerConnected(target)) return ER_Send(playerid, COLOR_GREY, "Invalid player.");
    if(isnull(weapon)) return ER_ShowGiveGunList(playerid, target);
    new wid = ER_WeaponNameToID(weapon);
    if(wid < 1 || wid > 46) return ER_Send(playerid, COLOR_GREY, "Invalid weapon. Use /givegun [player] [weapon id/name/partial] [ammo].");
    if(ammo < 1) ammo = 500;
    GivePlayerWeapon(target, wid, ammo);
    new msg[144]; format(msg, sizeof(msg), "You gave %s a %s with %d ammo.", ER_GetName(target), ER_GetWeaponNameEx(wid), ammo);
    return ER_Send(playerid, COLOR_GREEN, msg);
}
...
CMD:sethp(playerid, params[])
{
    ...
    SetPlayerHealth(target, hp); PlayerInfo[target][pHealth] = hp;
