...
CMD:deletejob(playerid, params[])
{
    ...
    ER_LoadJobs(); return ER_Send(playerid, COLOR_GREEN, "Job deleted/disabled.");
}
...
stock ER_JobDialog(playerid, dialogid, response, listitem, const inputtext[])
{
    ...
    if(dialogid == DIALOG_JOB_EDITOR)
    {
        ...
        if(listitem == 2)
        {
            ...
            mysql_format(MainPipeline, q, sizeof(q), "UPDATE `jobs` SET `x`=%f,`y`=%f,`z`=%f,`a`=%f,`interior`=%d,`vw`=%d WHERE `id`=%d", x,y,z,a,GetPlayerInterior(playerid),GetPlayerVirtualWorld(playerid),id); mysql_tquery(MainPipeline,q); ER_LoadJobs(); return ER_Send(playerid, COLOR_GREEN, "Job position saved.");
        }
        ...
        if(listitem == 4) { new q[128]; mysql_format(MainPipeline, q, sizeof(q), "UPDATE `jobs` SET `enabled`=0 WHERE `id`=%d", id); mysql_tquery(MainPipeline, q); ER_LoadJobs(); return ER_Send(playerid, COLOR_GREEN, "Job deleted/disabled."); }
        if(listitem == 5) { ER_LoadJobs(); return ER_Send(playerid, COLOR_GREEN, "Jobs reloaded."); }
        return 1;
    }
    if(dialogid == DIALOG_JOB_INPUT)
    {
        ...
        mysql_tquery(MainPipeline, q); ER_LoadJobs(); return ER_Send(playerid, COLOR_GREEN, "Job updated.");
    }
    return 0;
}
...
{
    new target, weapon, price;
    if(!ER_RequireJobType(playerid, JOB_TYPE_ARMS, "sellgun")) return 1;
    if(sscanf(params,"udd",target,weapon,price)) return ER_Send(playerid,COLOR_GREY,"USAGE: /sellgun [player] [weaponid] [price]");
    ...
    if(PlayerInfo[playerid][pMaterials] < 100) return ER_Send(playerid,COLOR_GREY,"You need at least 100 materials.");
    if(price < 0 || price > 50000) return ER_Send(playerid,COLOR_GREY,"Invalid price.");
    PlayerInfo[playerid][pMaterials] -= 100; GivePlayerWeapon(target, weapon, 50); ER_SaveCharacter(playerid);
    return ER_Send(playerid,COLOR_GREEN,"Weapon sold/created.");
}
