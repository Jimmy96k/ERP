...
public ER_OnGatesLoad()
{
    ...
    for(new r; r < rows && GateCount < MAX_GATES; r++)
    {
        ...
        cache_get_value_name_int(r,"owner_type",Gates[GateCount][gOwnerType]); cache_get_value_name_int(r,"owner_id",Gates[GateCount][gOwnerID]); cache_get_value_name_int(r,"rank",Gates[GateCount][gRank]); cache_get_value_name_int(r,"enabled",Gates[GateCount][gEnabled]);
...
stock ER_GetNearestGate(playerid)
{
    ...
    {
        if(vw == Gates[i][gVW] && interior == Gates[i][gInt] && (IsPlayerInRangeOfPoint(playerid, 8.0, Gates[i][gClosedX], Gates[i][gClosedY], Gates[i][gClosedZ]) || IsPlayerInRangeOfPoint(playerid, 8.0, Gates[i][gOpenX], Gates[i][gOpenY], Gates[i][gOpenZ]))) return i;
    }
    return -1;
}
...
stock ER_ToggleGate(playerid, idx)
{
    ...
    if(Gates[idx][gOpen]) MoveDynamicObject(Gates[idx][gObjectID], Gates[idx][gClosedX], Gates[idx][gClosedY], Gates[idx][gClosedZ], 2.5, Gates[idx][gClosedRX], Gates[idx][gClosedRY], Gates[idx][gClosedRZ]);
    else MoveDynamicObject(Gates[idx][gObjectID], Gates[idx][gOpenX], Gates[idx][gOpenY], Gates[idx][gOpenZ], 2.5, Gates[idx][gOpenRX], Gates[idx][gOpenRY], Gates[idx][gOpenRZ]);
...
{
    new id, idx;
    if(sscanf(params, "D(0)", id) || id == 0) idx = ER_GetNearestGate(playerid); else idx = ER_FindGateIndexBySQLID(id);
...
{
    new id=cache_insert_id(),msg[96]; ER_LoadGates(); format(msg,sizeof(msg),"Gate created with ID %d. Use /editgate %d.",id,id); return ER_Send(playerid,COLOR_GREEN,msg);
}
...
stock ER_ShowGateEditor(playerid, id)
{
    ...
    return ShowPlayerDialog(playerid,DIALOG_GATE_EDITOR,DIALOG_STYLE_LIST,"Gate Editor","Goto Gate\nEdit Model ID\nSet Closed Position Here\nSet Open Position Here\nEdit Closed Position Tool\nEdit Open Position Tool\nOpen = Close\nSet Owner Type\nSet Owner ID\nSet Rank\nToggle Gate\nDelete Gate","Select","Close");
}

stock ER_DeleteGateByID(playerid, id)
{
    new q[128]; mysql_format(MainPipeline,q,sizeof(q),"UPDATE `gates` SET `enabled`=0 WHERE `id`=%d",id); mysql_tquery(MainPipeline,q); ER_LoadGates(); return ER_Send(playerid,COLOR_GREEN,"Gate deleted/disabled.");
}
...
stock ER_GateDialog(playerid, dialogid, response, listitem, const inputtext[])
{
    ...
    if(dialogid == DIALOG_GATE_EDITOR)
    {
        ...
        if(listitem>=7 && listitem<=9) return ShowPlayerDialog(playerid,DIALOG_GATE_INPUT,DIALOG_STYLE_INPUT,"Gate Setting","Enter value:","Save","Back");
        if(listitem==10) return ER_ToggleGate(playerid,idx);
        if(listitem==11) return ShowPlayerDialog(playerid,DIALOG_GATE_DELETE_CONFIRM,DIALOG_STYLE_MSGBOX,"Delete Gate","Delete/disable this gate?","Delete","Cancel");
        return 1;
    }
    if(dialogid == DIALOG_GATE_INPUT)
    {
        if(!response) return ER_ShowGateEditor(playerid, GetPVarInt(playerid,"EditingGate")); new id=GetPVarInt(playerid,"EditingGate"), action=GetPVarInt(playerid,"GateEditAction"), q[160], val=strval(inputtext);
        if(action==1)
        {
        ...
        else if(action==7) mysql_format(MainPipeline,q,sizeof(q),"UPDATE `gates` SET `owner_type`=%d WHERE `id`=%d",val,id);
        else if(action==8) mysql_format(MainPipeline,q,sizeof(q),"UPDATE `gates` SET `owner_id`=%d WHERE `id`=%d",val,id);
        else if(action==9) mysql_format(MainPipeline,q,sizeof(q),"UPDATE `gates` SET `rank`=%d WHERE `id`=%d",val,id);
