...
stock ER_ShowDealershipVehicles(playerid, businessid)
{
    new list[2048], line[128], found;
    for(new i; i < DealershipDisplayCount; i++)
    {
        if(!DealershipDisplay[i][ddEnabled] || DealershipDisplay[i][ddBusinessID] != businessid) continue;
        format(line, sizeof(line), "(%d) %s - %s - Stock: %d\n", DealershipDisplay[i][ddSQLID], ER_GetVehicleModelName(DealershipDisplay[i][ddModel]), ER_FormatMoney(DealershipDisplay[i][ddPrice]), DealershipDisplay[i][ddStock]);
...
{
    new q[768];
    mysql_format(MainPipeline, q, sizeof(q), "INSERT INTO `business_products` (`business_id`,`catalog_id`,`product_name`,`product_key`,`price`,`min_price`,`max_price`,`restock_cost`,`material_cost`,`stock`,`stock_capacity`,`admin_enabled`,`owner_enabled`) SELECT %d,c.`id`,c.`product_name`,c.`product_key`,c.`price`,c.`min_price`,c.`max_price`,c.`restock_cost`,c.`material_cost`,c.`default_stock_capacity`,c.`default_stock_capacity`,1,1 FROM `business_product_catalog` c LEFT JOIN `business_products` bp ON bp.`business_id`=%d AND bp.`catalog_id`=c.`id` WHERE c.`business_type`=%d AND c.`enabled`=1 AND bp.`id` IS NULL", businessid, businessid, type);
    mysql_tquery(MainPipeline, q);
    return 1;
}
...
CMD:enter(playerid, params[])
{
    ...
    if(idx != -1)
    {
        ...
        if(Businesses[idx][bLockable] && Businesses[idx][bLocked]) return ER_Send(playerid, COLOR_GREY, "This business is locked.");
...
CMD:buy(playerid, params[])
{
    ...
    new q[768];
    mysql_format(MainPipeline, q, sizeof(q), "INSERT INTO `business_products` (`business_id`,`catalog_id`,`product_name`,`product_key`,`price`,`min_price`,`max_price`,`restock_cost`,`material_cost`,`stock`,`stock_capacity`,`admin_enabled`,`owner_enabled`) SELECT %d,c.`id`,c.`product_name`,c.`product_key`,c.`price`,c.`min_price`,c.`max_price`,c.`restock_cost`,c.`material_cost`,c.`default_stock_capacity`,c.`default_stock_capacity`,1,1 FROM `business_product_catalog` c LEFT JOIN `business_products` bp ON bp.`business_id`=%d AND bp.`product_key`=c.`product_key` WHERE c.`business_type`=%d AND c.`enabled`=1 AND bp.`id` IS NULL", Businesses[idx][bSQLID], Businesses[idx][bSQLID], Businesses[idx][bType]);
...
stock ER_ShowBusinessProductsAdmin(playerid, bid)
{
    ...
    new q[2048];
    mysql_format(MainPipeline, q, sizeof(q), "INSERT INTO `business_products` (`business_id`,`catalog_id`,`product_name`,`product_key`,`price`,`min_price`,`max_price`,`restock_cost`,`material_cost`,`stock`,`stock_capacity`,`admin_enabled`,`owner_enabled`) SELECT %d,c.`id`,c.`product_name`,c.`product_key`,c.`price`,c.`min_price`,c.`max_price`,c.`restock_cost`,c.`material_cost`,c.`default_stock_capacity`,c.`default_stock_capacity`,1,1 FROM `business_product_catalog` c LEFT JOIN `business_products` bp ON bp.`business_id`=%d AND bp.`product_key`=c.`product_key` WHERE c.`business_type`=%d AND c.`enabled`=1 AND bp.`id` IS NULL", bid, bid, Businesses[idx][bType]);
...
public ER_BizProdSyncCB(playerid, bid)
{
    ...
    new q[1024];
    mysql_format(MainPipeline, q, sizeof(q), "SELECT bp.`id`,bp.`product_name`,bp.`price`,bp.`min_price`,bp.`max_price`,bp.`restock_cost`,bp.`stock`,bp.`stock_capacity`,bp.`admin_enabled`,bp.`owner_enabled` FROM `business_products` bp JOIN `business_product_catalog` c ON c.`id`=bp.`catalog_id` WHERE bp.`business_id`=%d AND c.`business_type`=%d ORDER BY bp.`id` ASC LIMIT 64", bid, Businesses[idx][bType]);
    if(q[0] == EOS)
    {
...
stock ER_BusinessDialog(playerid, dialogid, response, listitem, const inputtext[])
{
    ...
    if(dialogid == DIALOG_MY_BUSINESS_MENU)
    {
        ...
        if(Businesses[idx][bType] == BUSINESS_TYPE_GAS)
        {
            ...
            if(listitem == 2) return ShowPlayerDialog(playerid, DIALOG_GAS_PUMP_LIST, DIALOG_STYLE_LIST, "Gas Pumps", "Use /editpumps to list all gas pumps. /editpump [id] for specific pump.", "Close", "");
            return ER_ShowBusinessEditor(playerid, bid);
        }
        ...
        if(Businesses[idx][bType] == BUSINESS_TYPE_DEALERSHIP)
        {
            switch(listitem)
            {
                ...
                case 3: return ER_ShowDealershipVehicles(playerid, bid);
        ...
        switch(listitem)
        {
            ...
            {
                SetPVarInt(playerid, "MyBizAction", 1);
                ShowPlayerDialog(playerid, DIALOG_MY_BUSINESS_INPUT, DIALOG_STYLE_INPUT, "Owner Deposit To Safe", "Enter cash amount to deposit into the business safe:", "Deposit", "Back");
                return 1;
            }
            case 3:
            {
                SetPVarInt(playerid, "MyBizAction", 2);
                ShowPlayerDialog(playerid, DIALOG_MY_BUSINESS_INPUT, DIALOG_STYLE_INPUT, "Owner Withdraw From Safe", "Enter amount to withdraw from the business safe:", "Withdraw", "Back");
                return 1;
            }
            ...
            case 7:
            {
                new refund = Businesses[idx][bPrice] / 2; PlayerInfo[playerid][pCash] += refund; GivePlayerMoney(playerid, refund); new q[256]; mysql_format(MainPipeline, q, sizeof(q), "UPDATE `businesses` SET `owner_type`=0,`owner_id`=0,`owner_name`='Nobody',`locked`=0 WHERE `id`=%d", bid); mysql_tquery(MainPipeline, q); ER_ReloadBusinessBySQLID(bid, playerid); return ER_Send(playerid, COLOR_GREEN, "Business sold for 50 percent of its price.");
            }
        }
        return 1;
    }
    ...
    if(dialogid == DIALOG_MY_BUSINESS_INPUT)
    {
        ...
        if(action >= 10 && action <= 12)
        {
            ...
            if(action == 10) mysql_format(MainPipeline, q, sizeof(q), "UPDATE `business_products` SET `price`=%d WHERE `id`=%d AND `business_id`=%d AND `admin_enabled`=1", amount, pid, bid);
            else if(action == 11) mysql_format(MainPipeline, q, sizeof(q), "UPDATE `business_products` SET `stock`=`stock`+%d WHERE `id`=%d AND `business_id`=%d AND `admin_enabled`=1", amount, pid, bid);
            else if(action == 12) mysql_format(MainPipeline, q, sizeof(q), "UPDATE `business_products` SET `stock`=GREATEST(`stock`-%d,0) WHERE `id`=%d AND `business_id`=%d AND `admin_enabled`=1", amount, pid, bid);
            mysql_tquery(MainPipeline, q);
    ...
    if(dialogid == DIALOG_MY_BUSINESS_PRODUCT_OPTIONS)
    {
        ...
        if(listitem == 0) ShowPlayerDialog(playerid, DIALOG_MY_BUSINESS_INPUT, DIALOG_STYLE_INPUT, "Change Product Price", "Enter new product price:", "Save", "Back");
        else if(listitem == 1) ShowPlayerDialog(playerid, DIALOG_MY_BUSINESS_INPUT, DIALOG_STYLE_INPUT, "Restock Product", "Enter amount of stock to restock:\nCost will be deducted from the business safe.", "Restock", "Back");
        else if(listitem == 2)
        {
            new pid = GetPVarInt(playerid, "MyBusinessProduct"), q[160];
            mysql_format(MainPipeline, q, sizeof(q), "UPDATE `business_products` SET `owner_enabled`=IF(`owner_enabled`=1,0,1) WHERE `id`=%d AND `admin_enabled`=1", pid);
            mysql_tquery(MainPipeline, q);
            ER_Send(playerid, COLOR_GREEN, "Product availability updated.");
            return ER_ShowMyBusinessProducts(playerid, GetPVarInt(playerid, "MyBusiness"));
        }
        return 1;
    }
    ...
    if(dialogid == DIALOG_BUSINESS_EDITOR)
    {
        ...
        switch(listitem)
        {
            ...
            case 17:
            {
                ...
                if(!Businesses[idx][bLockable]) Businesses[idx][bLocked] = 0;
                new q[160]; mysql_format(MainPipeline, q, sizeof(q), "UPDATE `businesses` SET `lockable`=%d,`locked`=%d WHERE `id`=%d", Businesses[idx][bLockable], Businesses[idx][bLocked], bid); mysql_tquery(MainPipeline, q); ER_UpdateBusinessLabel(idx);
...
stock ER_ShowDealershipVehicleEditor(playerid, dealerSqlId)
{
    ...
    format(list, sizeof(list), "Edit Vehicle Model: %s\nVehicle Mods\nMove Display Car To Me\nSet Custom Purchase Spawn To Me\nReset Purchase Spawn To Display Position\nSet Price: %s\nSet Stock / Capacity: %d\nSet Colors\nLink To Another Dealership\n%s\nDelete Vehicle", ER_GetVehicleModelName(DealershipDisplay[didx][ddModel]), ER_FormatMoney(DealershipDisplay[didx][ddPrice]), DealershipDisplay[didx][ddStock], DealershipDisplay[didx][ddEnabled] ? "Disable Vehicle" : "Enable Vehicle");
