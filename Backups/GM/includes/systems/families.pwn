...
CMD:f(playerid, params[])
{
    ...
    new msg[180], sendcolor = FAMILY_CHAT_COLOR_DEFAULT;
    ...
    format(msg, sizeof(msg), "(( Family )) %s: %s", ER_GetName(playerid), params);
...
CMD:crew(playerid, params[])
{
    ...
    new msg[180], sendcolor = FAMILY_CREW_COLOR_DEFAULT;
    new famidx = ER_FindFamilyIndexBySQLID(PlayerInfo[playerid][pFamily]);
    if(famidx != -1 && Families[famidx][fCrewColor] != 0) sendcolor = Families[famidx][fCrewColor];
    format(msg, sizeof(msg), "(( Crew )) %s: %s", ER_GetName(playerid), params);
...
stock ER_FamilyDialog(playerid, dialogid, response, listitem, inputtext[])
{
    ...
    if(dialogid == DIALOG_FAMILY_EDITOR)
    {
        ...
        switch(listitem)
        {
            ...
            case 1: { SetPVarInt(playerid, "FamilyInputField", 3); ShowPlayerDialog(playerid, DIALOG_FAMILY_INPUT, DIALOG_STYLE_INPUT, "Family Leader", "Enter leader account name, or 0/Nobody to clear:\nExample: Jimmy_Richardson", "Save", "Back"); }
