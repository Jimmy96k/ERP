...
stock ER_HouseDialog(playerid, dialogid, response, listitem, const inputtext[])
{
    ...
    if(dialogid == DIALOG_MY_HOUSE_MENU)
    {
        ...
        switch(listitem)
        {
            ...
            case 4: return ER_Send(playerid, COLOR_GREY, "House spawn setting is reserved for next code pass.");
            case 5:
            {
    ...
    if(dialogid == DIALOG_HOUSE_EDITOR)
    {
        ...
        if(listitem == 3) { new q[128]; mysql_format(MainPipeline,q,sizeof(q),"UPDATE `houses` SET `int_vw`=%d WHERE `id`=%d",hid,hid); mysql_tquery(MainPipeline,q); ER_ReloadHouseBySQLID(hid, playerid); return ER_Send(playerid,COLOR_GREEN,"House VW reset to House ID."); }
        ...
        if(listitem == 6) { ER_ReloadHouseBySQLID(hid, playerid); return ER_Send(playerid,COLOR_GREEN,"House reloaded."); }
