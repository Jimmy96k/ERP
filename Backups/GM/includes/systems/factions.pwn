...
stock ER_DepartmentChat(playerid, const text[])
{
    ...
    new msg[180], sendcolor = FACTION_CHAT_COLOR_DEFAULT;
    ...
    format(msg, sizeof(msg), "(( Department )) %s: %s", ER_GetName(playerid), text);
...
CMD:division(playerid, params[])
{
    ...
    new msg[180], sendcolor = FACTION_DIV_COLOR_DEFAULT;
    new facidx = ER_FindFactionIndexBySQLID(PlayerInfo[playerid][pFaction]);
    if(facidx != -1 && Factions[facidx][facDivisionColor] != 0) sendcolor = Factions[facidx][facDivisionColor];
    format(msg, sizeof(msg), "(( Division )) %s: %s", ER_GetName(playerid), params);
...
stock ER_FactionDialog(playerid, dialogid, response, listitem, inputtext[])
{
    ...
    if(dialogid == DIALOG_FACTION_EDITOR)
    {
        ...
        switch(listitem)
        {
            ...
            case 1: { SetPVarInt(playerid, "FactionInputField", 3); ShowPlayerDialog(playerid, DIALOG_FACTION_INPUT, DIALOG_STYLE_INPUT, "Faction Leader", "Enter leader account name, or 0/Nobody to clear:\nExample: Jimmy_Richardson", "Save", "Back"); }
            case 2: { SetPVarInt(playerid, "FactionInputField", 2); ShowPlayerDialog(playerid, DIALOG_FACTION_INPUT, DIALOG_STYLE_INPUT, "Faction MOTD", "Enter new faction MOTD:", "Save", "Back"); }
            case 3: { SetPVarInt(playerid, "FactionColorField", 1); ER_ShowNamedColorDialog(playerid, DIALOG_FACTION_COLOR_SELECT, "Department Chat Color"); }
            case 4: { SetPVarInt(playerid, "FactionColorField", 2); ER_ShowNamedColorDialog(playerid, DIALOG_FACTION_COLOR_SELECT, "Division Chat Color"); }
            case 5: return ER_ShowFactionRanksEditor(playerid, GetPVarInt(playerid, "EditingFaction"));
            case 6: return ER_ShowFactionDivisionsEditor(playerid, GetPVarInt(playerid, "EditingFaction"));
            case 7: return ER_ShowFactionLockerEditor(playerid, GetPVarInt(playerid, "EditingFaction"));
            case 8: return ER_ShowFactionLockerEditor(playerid, GetPVarInt(playerid, "EditingFaction"));
            case 9: return ER_ShowFactionPermsEditor(playerid, GetPVarInt(playerid, "EditingFaction"));
            case 10: { SetPVarInt(playerid, "FactionInputField", 50); ShowPlayerDialog(playerid, DIALOG_FACTION_INPUT, DIALOG_STYLE_LIST, "Faction Status", "Enabled\nDisabled", "Save", "Back"); }
            case 11: { ER_LoadFactions(); ER_Send(playerid, COLOR_GREEN, "Factions reloaded."); }
        }
        return 1;
    }
    ...
    if(dialogid == DIALOG_FACTION_LOCKERS)
    {
        ...
        else ER_Send(playerid, COLOR_GREY, "Gun locker weapon editor will be connected in the locker pass.");
        return ER_ShowFactionEditor(playerid, fid);
    }
