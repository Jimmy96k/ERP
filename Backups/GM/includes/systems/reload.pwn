...
stock ER_DoReloadSystem(playerid, const system[])
{
    ...
    if(!strcmp(system, "all", true)) { ER_LoadServerCore(); ER_LoadBusinesses(); ER_LoadVehicles(); ER_LoadDealershipDisplays(); ER_LoadHouses(); ER_LoadDoors(); ER_LoadFamilies(); ER_LoadFactions(); ER_LoadRadioStations(); ER_LoadAudioZones(); return ER_Send(playerid, COLOR_GREEN, "All supported systems reloaded."); }
    return ER_Send(playerid, COLOR_GREY, "USAGE: /reload [businesses/vehicles/houses/doors/families/factions/radio/audio/servercore/all]");
}
...
stock ER_ReloadDialog(playerid, dialogid, response, listitem, const inputtext[])
{
    ...
    switch(listitem)
    {
        ...
        case 4: return ER_DoReloadSystem(playerid, "families");
        case 5: return ER_DoReloadSystem(playerid, "factions");
        case 6: return ER_DoReloadSystem(playerid, "radio");
        case 7: return ER_DoReloadSystem(playerid, "audio");
        case 8: return ER_DoReloadSystem(playerid, "servercore");
        case 9: return ER_DoReloadSystem(playerid, "all");
    }
    return 1;
}
