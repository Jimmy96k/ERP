...
stock ER_DoorHasAccess(playerid, idx)
{
    switch(Doors[idx][dOwnerType])
    {
        ...
        case DOOR_OWNER_FAMILY: return PlayerInfo[playerid][pFamily] == Doors[idx][dOwnerID] && PlayerInfo[playerid][pFamilyRank] >= Doors[idx][dLockRank] && (Doors[idx][dFamilyCrew] == 0 || PlayerInfo[playerid][pFamilyCrew] == Doors[idx][dFamilyCrew]);
        case DOOR_OWNER_FACTION: return PlayerInfo[playerid][pFaction] == Doors[idx][dOwnerID] && PlayerInfo[playerid][pFactionRank] >= Doors[idx][dLockRank] && (Doors[idx][dFactionDivision] == 0 || PlayerInfo[playerid][pFactionDivision] == Doors[idx][dFactionDivision]);
...
stock ER_DoorDialog(playerid, dialogid, response, listitem, const inputtext[])
{
    ...
    if(dialogid == DIALOG_DOOR_EDITOR)
    {
        ...
        if(listitem==7){ER_LoadDoors();return ER_Send(playerid,COLOR_GREEN,"Doors reloaded.");}
        return ER_Send(playerid,COLOR_GREY,"This door editor item is reserved for next code pass.");
    }
