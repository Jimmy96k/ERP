...
stock ER_DoReloadSystem(playerid, const system[])
{
    ...
    if(!strcmp(system, "all", true)) { ER_LoadServerCore(); ER_LoadBusinesses(); ER_LoadGasPumps(); ER_LoadVehicles(); ER_LoadDealershipDisplays(); ER_LoadHouses(); ER_LoadDoors(); ER_LoadJobs(); ER_LoadMatruns(); ER_LoadGates(); ER_LoadPoints(); ER_LoadTurfs(); ER_LoadFamilies(); ER_LoadFactions(); ER_LoadRadioStations(); ER_LoadAudioZones(); return ER_Send(playerid, COLOR_GREEN, "All supported systems reloaded."); }
    return ER_Send(playerid, COLOR_GREY, "USAGE: /reload [businesses/vehicles/houses/doors/jobs/matruns/gates/points/turfs/families/factions/radio/audio/servercore/all]");
}
...
stock ER_ReloadDialog(playerid, dialogid, response, listitem, const inputtext[])
{
    ...
    switch(listitem)
    {
        ...
        case 13: return ER_DoReloadSystem(playerid, "servercore");
        case 14: return ER_DoReloadSystem(playerid, "all");
    }
    return 1;
}
