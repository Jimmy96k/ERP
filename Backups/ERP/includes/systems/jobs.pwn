...
stock ER_ClearMatruns()
{
    ...
    {
        if(Matruns[i][mrPickupID]) DestroyDynamicPickup(Matruns[i][mrPickupID]);
        if(Matruns[i][mrPickupLabelID]) DestroyDynamic3DTextLabel(Matruns[i][mrPickupLabelID]);
        if(Matruns[i][mrDropoffLabelID]) DestroyDynamic3DTextLabel(Matruns[i][mrDropoffLabelID]);
        Matruns[i][mrPickupID] = 0;
        Matruns[i][mrPickupLabelID] = Text3D:0;
        Matruns[i][mrDropoffLabelID] = Text3D:0;
    }
    MatrunCount = 0;
    return 1;
}
...
{
    new label[192];
    format(label, sizeof(label), "{FFFF00}%s\n{FFFFFF}Material Pickup\nPackages: %d | Price: %s\nUse /getmats", Matruns[idx][mrPickupName], Matruns[idx][mrPackageAmount], ER_FormatMoney(Matruns[idx][mrPackageCost]));
    Matruns[idx][mrPickupID] = CreateDynamicPickup(1271, 23, Matruns[idx][mrPickupX], Matruns[idx][mrPickupY], Matruns[idx][mrPickupZ], Matruns[idx][mrPickupVW], Matruns[idx][mrPickupInt]);
    ...
    format(label, sizeof(label), "{FFFF00}%s\n{FFFFFF}Material Dropoff\nReward: %d materials", Matruns[idx][mrDropoffName], Matruns[idx][mrMaterialReward]);
...
{
    ER_ClearMatruns();
    mysql_tquery(MainPipeline, "SELECT * FROM `job_matruns` WHERE `enabled`=1 ORDER BY `id` ASC", "ER_OnMatrunsLoad");
    return 1;
}
...
stock ER_AddPointIncome(pointid, amount)
{
    ...
    new q[192];
    mysql_format(MainPipeline, q, sizeof(q), "UPDATE `points` SET `safe_balance`=`safe_balance`+%d WHERE `id`=%d AND `enabled`=1 AND `owner_family`>0", amount, pointid);
...
stock ER_JobDialog(playerid, dialogid, response, listitem, const inputtext[])
{
    ...
    if(dialogid == DIALOG_MRUN_EDIT)
    {
        ...
        switch(listitem)
        {
            ...
            case 3:
            {
                ...
                mysql_tquery(MainPipeline, q); ER_LoadMatruns(); ER_Send(playerid, COLOR_GREEN, "Matrun pickup position saved."); return 1;
            }
            case 4:
            {
                new Float:x, Float:y, Float:z;
                ...
                mysql_tquery(MainPipeline, q); ER_LoadMatruns(); ER_Send(playerid, COLOR_GREEN, "Matrun dropoff position saved."); return 1;
            }
            case 5: return ShowPlayerDialog(playerid, DIALOG_MRUN_INPUT, DIALOG_STYLE_INPUT, "Package Amount", "Enter package amount, e.g. 10:", "Save", "Back");
            case 6: return ShowPlayerDialog(playerid, DIALOG_MRUN_INPUT, DIALOG_STYLE_INPUT, "Package Price", "Enter package price:", "Save", "Back");
            case 7: return ShowPlayerDialog(playerid, DIALOG_MRUN_INPUT, DIALOG_STYLE_INPUT, "Materials Reward", "Enter materials rewarded on delivery:", "Save", "Back");
            case 8: return ShowPlayerDialog(playerid, DIALOG_MRUN_INPUT, DIALOG_STYLE_INPUT, "Pickup Point ID", "Enter pickup point ID, or 0 for none:\nThe pickup point gets the full package price.", "Save", "Back");
            case 9: return ShowPlayerDialog(playerid, DIALOG_MRUN_INPUT, DIALOG_STYLE_INPUT, "Dropoff Point ID", "Enter dropoff/factory point ID, or 0 for none:", "Save", "Back");
            case 10: return ShowPlayerDialog(playerid, DIALOG_MRUN_INPUT, DIALOG_STYLE_INPUT, "Factory Cut Percent", "Enter percentage of the package price added to the dropoff/factory point safe:\nExample: 20", "Save", "Back");
            case 11:
            {
                mysql_format(MainPipeline, q, sizeof(q), "UPDATE `job_matruns` SET `enabled`=IF(`enabled`=1,0,1) WHERE `id`=%d", id);
                mysql_tquery(MainPipeline, q); ER_LoadMatruns(); ER_Send(playerid, COLOR_GREEN, "Matrun enabled state toggled."); return 1;
            }
            case 12: return ShowPlayerDialog(playerid, DIALOG_MRUN_DELETE, DIALOG_STYLE_MSGBOX, "Delete Material Run", "Disable/delete this material run?", "Delete", "Back");
        }
        return 1;
    }
    if(dialogid == DIALOG_MRUN_INPUT)
    {
        new id = GetPVarInt(playerid, "EditingMatrun"), action = GetPVarInt(playerid, "MatrunEditAction"), q[512], value;
        ...
        if(action == 0 || action == 1 || action == 2)
        {
            ...
            if(action == 0) mysql_format(MainPipeline, q, sizeof(q), "UPDATE `job_matruns` SET `name`='%e' WHERE `id`=%d", inputtext, id);
            else if(action == 1) mysql_format(MainPipeline, q, sizeof(q), "UPDATE `job_matruns` SET `pickup_name`='%e' WHERE `id`=%d", inputtext, id);
            else mysql_format(MainPipeline, q, sizeof(q), "UPDATE `job_matruns` SET `dropoff_name`='%e' WHERE `id`=%d", inputtext, id);
        }
        else
        {
            ...
            switch(action)
            {
                case 5: mysql_format(MainPipeline, q, sizeof(q), "UPDATE `job_matruns` SET `package_amount`=%d WHERE `id`=%d", value, id);
                case 6: mysql_format(MainPipeline, q, sizeof(q), "UPDATE `job_matruns` SET `package_cost`=%d WHERE `id`=%d", value, id);
                case 7: mysql_format(MainPipeline, q, sizeof(q), "UPDATE `job_matruns` SET `material_reward`=%d WHERE `id`=%d", value, id);
                case 8: mysql_format(MainPipeline, q, sizeof(q), "UPDATE `job_matruns` SET `pickup_point_id`=%d WHERE `id`=%d", value, id);
                case 9: mysql_format(MainPipeline, q, sizeof(q), "UPDATE `job_matruns` SET `dropoff_point_id`=%d WHERE `id`=%d", value, id);
                case 10:
                {
        ...
        mysql_tquery(MainPipeline, q); ER_LoadMatruns(); ER_Send(playerid, COLOR_GREEN, "Material run updated."); return ER_ShowMatrunEditor(playerid, id);
    }
    if(dialogid == DIALOG_MRUN_DELETE)
    {
        ...
        mysql_tquery(MainPipeline, q); ER_LoadMatruns(); ER_Send(playerid, COLOR_GREEN, "Material run deleted/disabled."); return 1;
    }
